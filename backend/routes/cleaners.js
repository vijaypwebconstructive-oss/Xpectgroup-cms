...
import { getOnboardingSuccessTemplate } from "../services/onboardingSuccessTemplate.js";
...
router.patch("/bulk-action", async (req, res) => {
  try {
    ...
    const validIds = cleanerIds.filter(
    ...
    Promise.all(
      cleaners.map((cleaner) =>
        sendVerificationStatusEmail(cleaner.email, cleaner.name, status).catch(
          (err) =>
            console.error(`Email failed for ${cleaner.email}`, err.message),
        ),
      ),
    );
  ...
  try {
    ...
    if (Object.keys(updates).length === 0) {
      return res.status(400).json({
        error: "Validation error",
        message:
          "Provide at least one of: hourlyPayRate, employmentType, location",
      });
    }
...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch cleaners", message: error.message });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch cleaner", message: error.message });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update cleaner", message: error.message });
  }
});
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        req.body.verificationStatus !== oldCleaner.verificationStatus
      ) {
        const oldStatus = oldCleaner.verificationStatus;
        const newStatus = req.body.verificationStatus;

        if (newStatus === "Verified") {
          await logVerificationActivity.verified(
            "admin-001",
            "Admin",
            cleaner.id,
            cleaner.name,
          ...
          await logVerificationActivity.rejected(
            "admin-001",
            "Admin",
            cleaner.id,
            cleaner.name,
      ...
      if (
        req.body.hourlyPayRate &&
        req.body.hourlyPayRate !== oldCleaner.hourlyPayRate
      ) {
      ...
      if (
        ...
        req.body.contractStatus !== oldCleaner.contractStatus
      ) {
      ...
      const hasEmploymentAllocationChanges = employmentAllocationFields.some(
        (field) =>
          req.body[field] !== undefined &&
          req.body[field] !== oldCleaner[field],
      );
      if (hasEmploymentAllocationChanges) {
        await logCleanerActivity.employmentAllocationUpdated(
          "admin-001",
          "Admin",
          cleaner.id,
          cleaner.name,
      ...
      const immigrationInfoFields = [
        "citizenshipStatus",
        "visaType",
        "shareCode",
      ];
      const hasImmigrationInfoChanges = immigrationInfoFields.some(
        (field) =>
          req.body[field] !== undefined &&
          req.body[field] !== oldCleaner[field],
      );
      if (hasImmigrationInfoChanges) {
        await logCleanerActivity.immigrationInfoUpdated(
          "admin-001",
          "Admin",
          cleaner.id,
          cleaner.name,
      ...
      if (
        req.body.auditorNotes !== undefined &&
        req.body.auditorNotes !== oldCleaner.auditorNotes
      ) {
        await logCleanerActivity.auditorNotesUpdated(
          "admin-001",
          "Admin",
          cleaner.id,
          cleaner.name,
        ...
        for (const newDoc of newDocuments) {
          ...
          if (oldDoc && oldDoc.status !== newDoc.status) {
            if (newDoc.status === "Verified") {
              await logDocumentActivity.verified(
                "admin-001",
                "Admin",
                cleaner.id,
                cleaner.name,
              ...
              await logDocumentActivity.rejected(
                "admin-001",
                "Admin",
                cleaner.id,
                cleaner.name,
              ...
              await logDocumentActivity.statusUpdated(
                "admin-001",
                "Admin",
                cleaner.id,
                cleaner.name,
      ...
      const hasPersonalDetailChanges = personalDetailFields.some(
        (field) =>
          req.body[field] !== undefined &&
          req.body[field] !== oldCleaner[field],
      );
      ...
      if (hasPersonalDetailChanges) {
        const changedFields = personalDetailFields.filter(
          (f) => req.body[f] !== undefined && req.body[f] !== oldCleaner[f],
        );
        await logCleanerActivity.updated(
          "admin-001",
          "Admin",
          cleaner.id,
          cleaner.name,
      ...
      const nameChanged =
        req.body.name !== undefined && req.body.name !== oldCleaner.name;
      const hourlyPayRateChanged =
        req.body.hourlyPayRate !== undefined &&
        req.body.hourlyPayRate !== oldCleaner.hourlyPayRate;
      const monthlySalaryChanged =
        req.body.monthlySalary !== undefined &&
        req.body.monthlySalary !== oldCleaner.monthlySalary;
      const payTypeChanged =
        req.body.payType !== undefined &&
        req.body.payType !== oldCleaner.payType;
      if (
        nameChanged ||
        hourlyPayRateChanged ||
        monthlySalaryChanged ||
        payTypeChanged
      ) {
        try {
          const unpaid = await PayrollRecord.find({
            workerId: cleaner.id,
          ...
          for (const p of unpaid) {
            ...
            if (payTypeChanged)
              p.payType = req.body.payType === "Monthly" ? "Monthly" : "Hourly";
          ...
          console.warn(
            "Failed to sync staff updates to payroll:",
            syncErr.message,
          );
        }
      }
...
router.get("/status/:status", async (req, res) => {
  try {
    const cleaners = await Cleaner.find({
      verificationStatus: req.params.status,
    }).sort({ createdAt: -1 });
    res.json(cleaners);
  } catch (error) {
    res
