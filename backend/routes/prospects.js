...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch prospects", message: err.message });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ error: "Not found", message: "Prospect not found" });
    res.json(doc);
  } catch (err) {
    console.error("Error fetching prospect:", err);
    res
      .status(500)
      .json({ error: "Failed to fetch prospect", message: err.message });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create prospect", message: err.message });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update prospect", message: err.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete prospect", message: err.message });
  }
});
