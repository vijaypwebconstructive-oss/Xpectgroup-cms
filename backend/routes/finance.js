...
import {
  ...
  saveSalarySlipPdf,
} from "../services/salarySlipPdf.js";
import { upload } from "../middleware/upload.js";
import { uploadPayslipService } from "../services/salarySlipService.js";
import {
...
router.get("/payroll", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch payroll", message: err.message });
  }
});

router.post("/salary-slips/upload", upload.single("file"), async (req, res) => {
  try {
    const { employeeId, date } = req.body;

    const result = await uploadPayslipService({
      employeeId,
      date,
      file: req.file,
    });
...
router.get("/payroll/:id", async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ error: "Not found", message: "Payroll record not found" });
    res.json(doc);
  } catch (err) {
    console.error("Error fetching payroll:", err);
    res
  ...
  try {
    ...
    if (body.workerId != null && body.workerName != null) {
      ...
      if (month == null || year == null || totalSalary == null) {
        return res.status(400).json({
          ...
          message:
            "workerId, workerName, month, year, totalSalary are required",
        });
      }
      ...
      const hw =
        hoursWorked != null ? parseFloat(hoursWorked) : isMonthly ? 0 : 160;
      const hr =
        hourlyRate != null ? parseFloat(hourlyRate) : isMonthly ? 0 : 12;
      const ms =
        monthlySalary != null
          ? parseFloat(monthlySalary)
          : isMonthly
            ? parseFloat(totalSalary)
            : null;
      ...
      if (existing) {
        return res.status(409).json({
          error: "Conflict",
          message:
            "Payroll for this cleaner already exists for the selected pay period.",
        });
      }
    ...
    for (const a of assignments) {
      ...
      if (isMonthly) {
        ...
        if (
          overrides[a.workerId] != null &&
          typeof overrides[a.workerId].hoursWorked === "number"
        ) {
    ...
    res
      .status(500)
      .json({ error: "Failed to generate payroll", message: err.message });
  }
});
...
//     const doc = await PayrollRecord.findById(id);
...
router.patch("/payroll/:id", async (req, res) => {
  try {
    ...
    if (paymentStatus === "Paid") {
      doc.paymentDate = paymentDate || new Date().toISOString().split("T")[0];
    }
...
router.delete("/payroll/:id", async (req, res) => {
  try {
    ...
    // ✅ delete payroll using _id
    const deletedPayroll = await PayrollRecord.findByIdAndDelete(id);
    console.log("deleted", deletedPayroll);

    if (!deletedPayroll) {
      return res.status(404).json({ message: "Payroll not found" });
    }

    // ✅ delete linked salary slip (IMPORTANT FIX)
    await SalarySlip.deleteOne({ payrollId: deletedPayroll._id });
...
router.get("/invoices", async (req, res) => {
  try {
    ...
    if (clientName)
      filter["billTo.clientName"] = new RegExp(String(clientName), "i");
    // if (year != null) {
    //   const y = parseInt(year, 10);
    //   filter.$or = [
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch invoices", message: err.message });
  }
});
...
router.get("/invoices/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch invoice", message: err.message });
  }
});
...
async function nextInvoiceNumber() {
  ...
  try {
    ...
    if (settings?.invoicePrefix)
      prefixTemplate = String(settings.invoicePrefix);
  } catch (_) {
    /* use default */
  }
...
router.post("/invoices", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create invoice", message: err.message });
  }
});
...
router.patch("/invoices/:id", async (req, res) => {
  try {
    ...
    for (const k of allowed) {
      ...
      if (k === "status")
        doc[k] = ["Paid", "Overdue", "Sent"].includes(body[k])
          ? body[k]
          : "Pending";
      else if (["billBy", "billTo"].includes(k) && typeof body[k] === "object")
        doc[k] = body[k];
      else if (k === "serviceDetails") {
        const sd = body[k];
        doc[k] = !sd ? [] : Array.isArray(sd) ? sd : [sd];
      } else if (k === "serviceItems" && Array.isArray(body[k]))
        doc[k] = body[k];
      else if (
        [
          "subtotal",
    ...
    res
      .status(500)
      .json({ error: "Failed to update invoice", message: err.message });
  }
});
...
router.delete("/invoices/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete invoice", message: err.message });
  }
});
...
router.get("/salary-slips", async (req, res) => {
  ...
  try {
    ...
    const docs = await SalarySlip.find(filter)
      .sort({ year: -1, month: -1, workerName: 1 })
      .lean();
    res.json(docs);
  } catch (err) {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch salary slips", message: err.message });
  }
});
...
router.get("/salary-slips/:id", async (req, res) => {
  try {
    ...
    const doc = await SalarySlip.findById(req.params.id);
    if (!doc)
      return res
        .status(404)
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch salary slip", message: err.message });
  }
});
...
router.get("/salary-slips/:id/download", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to download salary slip", message: err.message });
  }
});
  ...
  try {
    ...
    if (!email) {
      return res.status(400).json({
        error: "Validation error",
        message:
          "Cleaner has no email address. Please add email in staff detail page.",
      });
    }

    const payrollDoc = await PayrollRecord.findById(data.id).lean();
    ...
    const pdfFilename = `salary-slip-${data.period.replace(/\s/g, "-")}.pdf`;
    const result = await sendSalarySlipWithPdf(
      email,
      data.cleanerName,
      record,
      fullPath,
    ...
    res
      .status(500)
      .json({ error: "Failed to send salary slip", message: err.message });
  }
});

// router.post('/salary-slips/:id/send', async (req, res) => {
//   try {
//     const data = req.body; // ✅ no await
//     console.log("7")

//     // ✅ Use ID from params or body
//     const slipId = data?.salarySlip?._id || req.params.id;

//     // ✅ Correct DB query
//     const slip = await SalarySlip.findById(slipId).lean();
...
//       return res.status(400).json({
//         error: 'Validation error',
//         message: 'Cleaner has no email address'
//       });
//     }
...
router.get("/site-contracts", async (req, res) => {
  try {
    const docs = await SiteContract.find()
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch site contracts", message: err.message });
  }
});
...
router.get("/site-contracts/:id", async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ error: "Not found", message: "Site contract not found" });
    res.json(doc);
  } catch (err) {
    console.error("Error fetching site contract:", err);
    res
      .status(500)
      .json({ error: "Failed to fetch site contract", message: err.message });
  }
});
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: "Validation error",
        message:
          "siteId, clientId, siteName, clientName, contractValue are required",
      });
    }
    ...
    if (existing) {
      return res.status(409).json({
        error: "Conflict",
        message: "A revenue contract already exists for this client and site.",
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to create site contract", message: err.message });
  }
});
...
router.patch("/site-contracts/:id", async (req, res) => {
  try {
    ...
    if (Array.isArray(paymentDocuments))
      doc.paymentDocuments = paymentDocuments;
    await doc.save();
    ...
    res
      .status(500)
      .json({ error: "Failed to update site contract", message: err.message });
  }
});
...
function normalizeQuotationDoc(doc) {
  ...
  if (!d.billTo.clientAddress && d.serviceAddress)
    d.billTo.clientAddress = d.serviceAddress;
  if (
    (!d.serviceItems || d.serviceItems.length === 0) &&
    ...
    const amount =
      Math.round(nc * hpv * vpw * 4 * (d.hourlyRate || 0) * 100) / 100;
    d.serviceItems = [
      {
...
router.get("/quotations", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch quotations", message: err.message });
  }
});
...
router.get("/quotations/:id", async (req, res) => {
  try {
    ...
    if (!doc)
      return res
        .status(404)
        .json({ error: "Not found", message: "Quotation not found" });
    res.json(normalizeQuotationDoc(doc));
  } catch (err) {
    console.error("Error fetching quotation:", err);
    res
      .status(500)
      .json({ error: "Failed to fetch quotation", message: err.message });
  }
});
...
router.post("/quotations", async (req, res) => {
  try {
    ...
    if (hasNewFormat) {
      ...
      const issueDate = formatDate(body.issueDate);
      const expiryDate = formatDate(body.expiryDate);
      let qn = body.quotationNumber;
      if (!qn) {
        const last = await Quotation.findOne().sort({ createdAt: -1 }).lean();
        const seq = last
          ? (parseInt(last.quotationNumber.replace(/\D/g, ""), 10) || 0) + 1
          : 1;
    ...
    if (!clientName || !clientEmail || hourlyRate == null) {
      return res.status(400).json({
        error: "Validation error",
        message: "clientName, clientEmail, hourlyRate are required",
      });
    }
    ...
    const computedPrice =
      totalPrice != null
        ...
        : Math.round(nc * hpv * vpw * 4 * hr * 100) / 100;
    ...
    res
      .status(500)
      .json({ error: "Failed to create quotation", message: err.message });
  }
});
...
router.patch("/quotations/:id", async (req, res) => {
  try {
    ...
    for (const k of allowed) {
      if (body[k] === undefined) continue;
      if (k === "status")
        doc[k] = ["Sent", "Accepted", "Rejected"].includes(body[k])
          ? body[k]
          : "Draft";
      else if (["billBy", "billTo"].includes(k) && typeof body[k] === "object")
        doc[k] = body[k];
    ...
    res
      .status(500)
      .json({ error: "Failed to update quotation", message: err.message });
  }
});
...
router.delete("/quotations/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete quotation", message: err.message });
  }
});
