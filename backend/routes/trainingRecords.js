...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch record", message: err.message });
  }
});
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!body.name || !body.course || !body.trainingStartDate) {
      return res
        .status(400)
        .json({ error: "name, course, and trainingStartDate are required" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to create record", message: err.message });
  }
});
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update record", message: err.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete record", message: err.message });
  }
});
