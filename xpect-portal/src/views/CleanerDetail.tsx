...
import EmploymentAllocationCard, {
  PayType,
  ShiftType,
  ContractStatus,
  EmploymentDetails,
...
const CleanerDetail: React.FC<CleanerDetailProps> = ({
  cleaner,
  onNavigate,
}) => {
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-6 animate-in fade-in duration-500">
        <div className="flex items-center gap-2 mb-4 text-[#4c669a]">
          <button
            ...
            className="hover:text-[#135bec] text-sm"
          >
            Dashboard
          </button>
  ...
  const [verificationStatus, setVerificationStatus] = useState(
    currentCleaner.verificationStatus,
  );
  const [auditorNotes, setAuditorNotes] = useState(
    currentCleaner.auditorNotes || "",
  );
  const [viewingDocument, setViewingDocument] = useState<Document | null>(null);
  const [documentStatusUpdates, setDocumentStatusUpdates] = useState<
  ...
  const mapPreferredShiftPatternToShiftType = (
  ...
  const handleStatusUpdate = async () => {
    try {
      ...
      alert(
        `Failed to update verification status: ${error.message || "Unknown error"}`,
      );
    }
  };
  ...
  const handleRevokeVerification = async () => {
    if (
      confirm(
        "Are you sure you want to revoke verification for this staff member?",
      )
    ) {
      try {
        await updateCleaner(currentCleaner.id, {
  ...
  const handleDelete = async () => {
    if (
      confirm(
        `Are you sure you want to delete ${currentCleaner.name}? This action cannot be undone and will permanently remove all their data.`,
      )
    ) {
      try {
        ...
        alert(
          `Failed to delete staff member: ${error.message || "Unknown error"}`,
        );
      }
    }
  };
  ...
  const needsShareCode =
    currentCleaner.citizenshipStatus !== "UK Citizen" &&
    currentCleaner.citizenshipStatus !== "Irish Citizen";
    ...
    if (!currentCleaner.documents || currentCleaner.documents.length === 0) {
      const defaultDocuments: Document[] = [
        ...
        },
        {
          ...
          uploadDate: new Date().toISOString().split("T")[0],
          status:
            currentCleaner.dbsStatus === "Cleared"
  ...
  const handleViewDocument = (doc: Document) => {
    ...
    const latestDoc =
      currentCleaner.documents?.find((d) => d.id === doc.id) || doc;
    ...
    if (latestDoc.fileUrl) {
      ...
      const shouldUpload = confirm(
        `Document "${latestDoc.name}" does not have a file uploaded. Would you like to upload it now?`,
      );
      if (shouldUpload) {
  ...
  const handleSaveDocumentStatuses = async () => {
    if (currentCleaner.documents) {
      try {
        ...
        alert(
          `Failed to update document statuses: ${error.message || "Unknown error"}`,
        );
      }
    }
  };
    ...
    try {
      ...
      await updateCleaner(currentCleaner.id, {
        location: details.workLocation
          ? `${details.workLocation.siteName}, ${details.workLocation.city}`
          : currentCleaner.location,
      ...
      alert(
        `Failed to update employment details: ${error.message || "Unknown error"}`,
      );
    }
  };
  ...
  const handleFileUpload = async (docId: string, file: File) => {
    ...
    reader.onloadend = () => {
      ...
      if (currentCleaner.documents) {
        ...
        updateCleaner(currentCleaner.id, { documents: updatedDocuments })
          .then(() => {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-6 py-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-2 mb-4 text-[#4c669a]">
        ...
        <span className="material-symbols-outlined text-sm text-[#0d121b]">
          chevron_right
        </span>
        <span className="text-[#0d121b] text-sm font-semibold">
          {cleaner.name} Profile
        </span>
      </div>
          ...
          <div className="flex gap-4 items-start">
            ...
            <div className="flex flex-col w-[60%]">
              <div className="flex  gap-3 flex-col sm:items-start">
                <h1 className="text-[#0d121b] text-2xl font-bold font-black">
                  {cleaner.name}
                </h1>
                <span
                  className={`px-3 py-1 text-[10px] w-fit font-black rounded-full uppercase tracking-widest ${
                    cleaner.verificationStatus === VerificationStatus.VERIFIED
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-12">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm font-bold text-gray-900">
                    {currentCleaner.dob || "Not provided"}
                  </p>
                )}
              </div>
              <div>
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm font-bold text-gray-900">
                    {currentCleaner.phoneNumber || "Not provided"}
                  </p>
                )}
              </div>
              <div>
              ...
              <div className="col-span-1 md:col-span-2">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm font-bold text-gray-900 leading-relaxed">
                    {currentCleaner.address || "No address on file"}
                  </p>
                )}
              </div>
              {isEditing && (
          ...
          <div className="bg-white border border-[#e7ebf3] rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 sm:px-6 py-4 border-b border-[#e7ebf3] bg-gray-50/30 flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold font-black uppercase tracking-widest text-gray-900">
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 sm:gap-y-6 gap-x-12">
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-900">
                    {currentCleaner.citizenshipStatus}
                  </p>
                </div>
                {hasVisa && (
                  <div>
                    ...
                    <p className="text-sm font-bold text-[#135bec]">
                      {currentCleaner.visaType}
                      {currentCleaner.visaOther
                        ? ` (${currentCleaner.visaOther})`
                        : ""}
                    </p>
                  </div>
                ...
                {needsShareCode && (
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <code className="bg-gray-100 px-3 py-1 rounded text-sm font-black tracking-widest text-gray-700">
                        {currentCleaner.shareCode || "PENDING"}
                      </code>
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-900">
                    {currentCleaner.workPreference || "Standard"}
                  </p>
                </div>
              </div>
              ...
              {isStudent && (
                <div className="mt-4 p-5 bg-blue-50/50 border border-blue-100 rounded-2xl space-y-4">
                  <h3 className="text-[12px] font-black uppercase tracking-widest text-[#135bec]">
                    Academic Details & Hours Compliance
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <p className="text-[12px] text-blue-900/60 font-black uppercase tracking-tight">
                        Institution
                      </p>
                      <p className="text-xs font-bold text-blue-900">
                        {currentCleaner.uniName}
                      </p>
                    </div>
                    <div>
                      <p className="text-[12px] text-blue-900/60 font-black uppercase tracking-tight">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-blue-900">
                        {currentCleaner.termStart} to {currentCleaner.termEnd}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[12px] font-black text-green-700 uppercase tracking-tight">
                        20HR Max Limit Agreed
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white border border-[#e7ebf3] rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 sm:px-6 py-4 border-b border-[#e7ebf3] flex flex-col sm:flex-row gap-4 align-start sm:align-center justify-start sm:justify-between bg-gray-50/30">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-[14px] font-bold font-black uppercase tracking-widest text-gray-900">
            ...
            <div className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              {(currentCleaner.documents || []).map((doc) => {
                const currentStatus =
                  documentStatusUpdates[doc.id] || doc.status;
                const statusColors = {
                ...
                return (
                  <div
                    key={doc.id}
                    className="border border-[#e7ebf3] rounded-2xl p-4 flex flex-col gap-4 hover:border-[#c7c7c7] transition-all bg-white group shadow-sm"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-bold font-black text-gray-700 leading-tight group-hover:text-[#000] transition-colors">
                            {doc.name}
                          </h3>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold font-black rounded ${statusColors[currentStatus]}`}
                        >
                          {currentStatus}
                        </span>
                        <select
                        ...
                        >
                          <option value={DocumentStatus.PENDING}>
                            Pending
                          </option>
                          <option value={DocumentStatus.VERIFIED}>
                            Verified
                          </option>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        title={
                          doc.fileUrl
                            ? "View document"
                            : "No file uploaded - click to upload"
                        }
                        disabled={!doc.fileUrl}
                      >
                        <span className="material-symbols-outlined text-sm">
                          {doc.fileUrl ? "visibility" : "visibility_off"}
                        </span>
                      ...
                      {doc.fileUrl ? (
                        ...
                        >
                          <span className="material-symbols-outlined text-sm">
                            {uploadingDocument?.docId === doc.id
                              ? "hourglass_empty"
                              : "upload"}
                          </span>
                        </button>
              ...
              {(!currentCleaner.documents ||
                currentCleaner.documents.length === 0) && (
          ...
          <div className="bg-white border border-[#e7ebf3] rounded-2xl overflow-hidden shadow-sm">
            <div className="px-4 sm:px-6 w-full py-4 border-b border-[#e7ebf3] bg-gray-50/30 flex items-center gap-2">
              ...
              <h2 className="text-[15px]  w-full sm:text-[16px] font-bold font-black uppercase tracking-widest text-gray-900">
            ...
            <div className="p-4 sm:p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                {
                  label: "Accuracy",
                  val: currentCleaner.declarations.accuracy,
                },
                { label: "RTW Consent", val: currentCleaner.declarations.rtw },
                {
                  label: "Approval Sub.",
                  val: currentCleaner.declarations.approval,
                },
                {
                  label: "GDPR Consent",
                  val: currentCleaner.declarations.gdpr,
                },
              ].map((dec) => (
                <div
                  key={dec.label}
                  className={`p-4 rounded-2xl border flex flex-col items-center text-center gap-2 ${dec.val ? "bg-green-50/50 border-green-100" : "bg-red-50/50 border-red-100"}`}
                >
                  <span
                    className={`material-symbols-outlined ${dec.val ? "text-green-600" : "text-red-600"}`}
                  >
                    {dec.val ? "check_circle" : "cancel"}
                  </span>
                  <p className="text-[10px] font-black uppercase tracking-tight text-gray-600">
                    {dec.label}
                  </p>
                </div>
              ))}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-[#e7ebf3] rounded-2xl p-6 shadow-sm sticky top-[90px]">
            ...
            <form className="space-y-6">
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-[#4c669a] mb-2">
                ...
                >
                  ...
                  <option value={VerificationStatus.PENDING}>
                    Pending Review
                  </option>
                  <option value={VerificationStatus.DOCS_REQUIRED}>
                    Docs Required
                  </option>
                  <option value={VerificationStatus.REJECTED}>
                    Rejected / Closed
                  </option>
                </select>
              </div>
              <div>
                <label className="block text-[14px] font-black  tracking-widest text-[#4c669a] mb-2">
              ...
              {viewingDocument.fileUrl ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-2">
                    The document file has not been uploaded yet.
                  </p>
                  <button
                    onClick={() => {
