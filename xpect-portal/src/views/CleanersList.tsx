...
const CleanersList: React.FC<CleanersListProps> = ({
  cleaners,
  onNavigate,
}) => {
  ...
  const pendingCount = completedCleaners.filter(
    (c) => c.verificationStatus === VerificationStatus.PENDING,
  ).length;
  const verifiedCount = completedCleaners.filter(
  ...
  const stats = [
    {
      label: "Total Staff",
      value: completedCleaners.length.toLocaleString(),
      trend: "Real-time",
      icon: "group",
      color: "blue",
      bgColor: "#eff6ff",
  ...
  const applyBulkAction = () => {
    ...
    if (selectedBulkAction === "EDIT_SELECTED") {
      if (selectedIds.size === 0) {
        setFeedback({
          type: "error",
          message: "Select at least one staff to edit.",
        });
        setTimeout(() => setFeedback(null), 4000);
        return;
      }
    ...
    if (selectedBulkAction === "DELETE") {
      if (selectedIds.size === 0) {
        setFeedback({
          type: "error",
          message: "Select at least one staff to delete.",
        });
        setTimeout(() => setFeedback(null), 4000);
        return;
      }
  ...
  const applyBulkEdit = async () => {
    const ids =
      selectedIds.size > 0
        ...
        : filteredCleaners.map((c) => c.id);
    if (ids.length === 0) {
      setFeedback({ type: "error", message: "No staff to update." });
      return;
    }
    const updates: { hourlyPayRate?: number; employmentType?: string } = {};
    if (bulkEditForm.employmentType)
      updates.employmentType = bulkEditForm.employmentType;
    ...
    if (rate !== "") {
      ...
      if (Number.isNaN(num) || num < 0) {
        setFeedback({
          type: "error",
          message: "Hourly pay must be a non-negative number.",
        });
        return;
      }
    ...
    if (!hasUpdates) {
      setFeedback({
        type: "error",
        message:
          "Set at least one field (employee type, status, or hourly pay).",
      });
      return;
    }
    ...
    try {
      ...
      if (hasStatus) {
        await api.cleaners.bulkUpdateStatus(
          ids,
          bulkEditForm.status as "Verified" | "Rejected" | "Pending",
        );
      }
  ...
  const applyEditSelected = async () => {
    ...
    if (editSelectedForm.employmentType)
      updates.employmentType = editSelectedForm.employmentType;
    if (editSelectedForm.workLocation.trim())
      updates.location = editSelectedForm.workLocation.trim();
    ...
    if (!hasUpdates) {
      setFeedback({
        type: "error",
        message:
          "Set at least one field (status, employee type, work location, or hourly pay).",
      });
      return;
    }
    ...
    try {
      ...
      if (hasStatus) {
        await api.cleaners.bulkUpdateStatus(
          ids,
          editSelectedForm.status as "Verified" | "Rejected" | "Pending",
        );
      }
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      "download",
      `xpect-staff-export-${new Date().toISOString().split("T")[0]}.csv`,
    );
      ...
      <div className="  space-y-6 ">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-col gap-1">
            <h1 className="text-[#0d121b] text-[1.6rem] sm:text-2xl font-black font-bold">
              Staff Management
            </h1>
            <p className="text-[#4c669a] text-base">
              Manage and monitor your team's compliance and onboarding status.
            </p>
          </div>
          <div className="flex gap-3 items-center">
            ...
            >
              ...
              <span className="truncate font-semibold font-[14px] font-[]">
                Export
              </span>
            </button>
            <button
          ...
          {stats.map((stat) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`p-2.5 sm:p-4 text-${stat.color}-600 rounded-lg `}
                  style={{
        ...
        {filteredCleaners.length > 0 ? (
          <div className="bg-white rounded-2xl border border-[#e7ebf3] shadow-sm overflow-hidden employee-list">
            ...
            <div className="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-[#e7ebf3] bg-gray-50/50">
              ...
              <button
                ...
                disabled={
                  selectedIds.size === 0 && selectedBulkAction !== "BULK_EDIT"
                }
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 border-b border-[#e7ebf3] text-[#4c669a] text-[10px] uppercase tracking-widest font-black">
                    ...
                    <th className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b] capitalize">
                      Staff Member
                    </th>
                    <th className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b] capitalize">
                      Status
                    ...
                    <th className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b] normal-case">
                      Pay(hr)
                    </th>
                    <th className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b] capitalize">
                      Location
                    ...
                    <th className="px-[16px] sm:px-6 py-4 text-right text-sm font-medium text-[#0d121b] capitalize">
                      Actions
                    </th>
                  </tr>
                </thead>
                  ...
                  {filteredCleaners.map((cleaner) => (
                    ...
                    >
                      ...
                      <td className="px-[16px] sm:px-6 py-4">
                        <div className="flex items-center gap-3">
                          {cleaner.avatar ? (
                            ...
                            <div
                              className={`size-12 sm:size-14 rounded-full shrink-0 flex items-center justify-center ${getAvatarColor(cleaner.id)}`}
                            >
                          ...
                          <div>
                            <p className="text-base font-bold text-[#0d121b]">
                              {cleaner.name}
                            </p>
                      ...
                      <td className="px-[16px] td-verifcation sm:px-6 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${getStatusStyles(cleaner.verificationStatus)}`}
                        >
                      ...
                      <td className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b]">
                        {cleaner.hourlyPayRate
                          ? `£${cleaner.hourlyPayRate.toFixed(2)}`
                          : "N/A"}
                      </td>
                      <td className="px-[16px] sm:px-6 py-4 text-sm font-medium text-[#0d121b]">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${cleaner.onboardingProgress}%`,
                              }}
                            ></div>
                          </div>
                          <span className="text-xs font-bold text-gray-500">
                            {cleaner.onboardingProgress}%
                          </span>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-2xl border border-[#e7ebf3] p-20 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <h3 className="text-xl font-bold font-black text-gray-900">
              No staff available
            </h3>
          ...
          >
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
              <h2
                ...
                className="text-lg font-black text-[#0d121b] mb-2"
              >
                Delete staff
              </h2>
        ...
        {bulkEditOpen && (
          ...
          >
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
              <h2
                id="bulk-edit-title"
                className="text-lg font-black text-[#0d121b] mb-1"
              >
                Bulk edit
              </h2>
              <p className="text-[#4c669a] text-sm mb-4">
                Update the following for{" "}
                {selectedIds.size > 0
                  ? selectedIds.size
              ...
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-sm font-bold text-[#0d121b] mb-1.5">
                    Employee type
                  </label>
                  <select
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setBulkEditForm((f) => ({ ...f, status: e.target.value }))
                    }
                    className="w-full h-11 px-4 rounded-lg border border-[#e7ebf3] bg-[#f6f6f8] text-[#0d121b] text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#2e4150]"
                  >
        ...
        {editSelectedOpen && (
          ...
          >
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
              <h2
                id="edit-selected-title"
                className="text-lg font-black text-[#0d121b] mb-1"
              >
                Edit the selected
              </h2>
              <p className="text-[#4c669a] text-sm mb-4">
                Update the following for {selectedIds.size} selected staff.
                Leave a field empty or "No change" to keep current value.
              </p>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block text-sm font-bold text-[#0d121b] mb-1.5">
