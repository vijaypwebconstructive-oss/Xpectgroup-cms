...
const EMAIL_STATUS_STYLES: Record<
  PPEInvoiceEmailStatus,
...
const isPreviewable = (record: PPEInvoiceRecord): boolean => {
  ...
  const ext = record.invoiceFile
    ? PREVIEWABLE_EXT.test(record.invoiceFile)
    : false;
  const mime =
    record.invoiceFileData.startsWith("data:") &&
    PREVIEWABLE_MIME.test(record.invoiceFileData.split(";")[0]);
  return ext || mime;
};
const isPdf = (record: PPEInvoiceRecord): boolean => {
  ...
  return (
    lower.endsWith(".pdf") ||
    record.invoiceFileData.startsWith("data:application/pdf")
  );
};

const PPEInvoiceList: React.FC<PPEInvoiceListProps> = ({ onAddRecord }) => {
  const { ppeInvoiceRecords, updatePPEInvoiceRecord, loading, error } =
    usePPEInvoice();
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-5 py-14 text-center">
                    ...
                    <p className="text-[#9aa5be] text-xs mt-1">
                      Click "Add PPE Record" to create your first invoice
                      record.
                    </p>
                  </td>
                </tr>
                ...
                ppeInvoiceRecords.map((record) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4">
                        {record.invoiceFileData ? (
                          ...
                          >
                            ...
                            <span
                              className="material-symbols-outlined text-[16px] text-[#4c669a] group-hover:text-[#2e4150]"
                              title="View"
                            >
                              visibility
                            </span>
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold ${statusStyle.badge}`}
                        >
                          {statusStyle.label}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-right">
                        <button
                          ...
                          disabled={
                            sendingId === record.id || !record.invoiceFileData
                          }
          ...
          <div className="px-5 py-3 border-t border-[#e7ebf3] bg-[#f8fafc] flex items-center justify-between flex-wrap gap-2">
            <p className="text-xs text-[#4c669a]">
              ...
              <span className="font-bold text-[#0d121b]">
                {ppeInvoiceRecords.length}
              </span>{" "}
              records
            </p>
            {sendMessage && (
              <p
                className={`text-xs font-medium ${sendMessage.includes("Failed") || sendMessage.includes("No ") ? "text-red-600" : "text-green-600"}`}
              >
                {sendMessage}
              </p>
            )}
