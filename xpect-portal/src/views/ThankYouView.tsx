...
const ThankYouView: React.FC<ThankYouViewProps> = ({
  ...
  const [formdataReferes, setFormdataReferes] = useState(0);
      ...
      try {
        ...
        setApplicationRef(
          parsed.applicationRef ||
            `XPG-${Math.random().toString(36).substring(2, 9).toUpperCase()}`,
        );
      } catch (e) {
      ...
      <div className="max-w-[640px] w-full bg-white rounded-[2.5rem] shadow-2xl shadow-blue-900/10 border border-gray-100 p-4 sm:p-10 md:p-16 flex flex-col items-center text-center ">
        ...
        <div className="relative mb-4">
          ...
          <div className="relative size-16 bg-[#34d399] text-white rounded-full flex items-center justify-center">
            <span className="material-symbols-outlined text-2xl font-bold animate-in zoom-in duration-500 delay-200 fill-0">
              check_circle
            </span>
          </div>
        </div>

        <h1 className="text-xl sm:text-2xl font-bold font-black text-gray-900 mb-4">
          Application Submitted!
        </h1>
        <p className="text-base sm:text-lg text-[#4c669a] font-medium leading-relaxed mb-4">
          Thank you for applying to join{" "}
          <span className=" font-bold">Xpect Group</span>. Your profile has been
          sent to our compliance team for review.
        </p>
        ...
        <div className="w-full bg-amber-50 border-2 border-amber-100/50 rounded-3xl p-4 sm:p-6 flex flex-col md:flex-row items-center md:items-start gap-4 text-left">
          ...
          <div>
            ...
            <p className="text-xs text-amber-800 font-medium leading-relaxed">
              Before your official start date, you{" "}
              <span className="font-black underline">must</span> provide a{" "}
              <span className="font-black">physical copy</span> of all your
              documents (Passport, BRP, DBS Certificate, etc.) to your
              respective <span className="font-black">Xpect Group office</span>{" "}
        ...
        <div className="mt-8 pt-8 border-t border-gray-100 w-full flex flex-col items-center gap-2">
          ...
          <code className="bg-gray-50 px-4 py-1.5 rounded-full text-xs font-black text-gray-500 tracking-widest border border-gray-100">
            {applicationRef ||
              `XPG-${Math.random().toString(36).substring(2, 9).toUpperCase()}`}
          </code>
        </div>
