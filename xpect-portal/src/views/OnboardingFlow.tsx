...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({
  ...
  const saveProgressOnStepChange = async (
    currentStep: number,
  ...
  const loadSavedProgress = async (token?: string): Promise<number> => {
    ...
    try {
      console.log(
        `[OnboardingFlow] Loading progress for inviteToken: ${tokenToUse}`,
      );
      ...
      if (response.success && response.hasProgress && response.progress) {
        ...
        const savedCurrentStep =
          saved.currentStep || saved.lastCompletedStep || 1;
        console.log(
        ...
        const base64ToFile = (
          ...
          try {
            ...
            console.warn(
              `Failed to convert base64 to File for ${filename}:`,
              err,
            );
            return null;
          }
        ...
        if (formData.shareCodeScreenshot && formData.shareCodeScreenshotName) {
          const file = base64ToFile(
            formData.shareCodeScreenshot,
            formData.shareCodeScreenshotName,
          );
        ...
        if (formData.residenceCard && formData.residenceCardName) {
          const file = base64ToFile(
            formData.residenceCard,
            formData.residenceCardName,
          );
        ...
        const restoredIsBritishOrIrish =
          restoredCitizenship === "UK Citizen" ||
          restoredCitizenship === "Irish Citizen";
        const restoredIsNonEU =
          restoredCitizenship === "Non-EU Citizen (Visa / BRP holder)";
        ...
        if (!restoredActiveSteps.includes(savedCurrentStep)) {
          ...
          if (
            ...
            lastCompletedIndex < restoredActiveSteps.length - 1
          ) {
            ...
            const closestIndex = restoredActiveSteps.findIndex(
              (s) => s >= lastCompleted,
            );
            resumeStep =
              closestIndex >= 0
                ? restoredActiveSteps[closestIndex]
                : restoredActiveSteps[0];
          }
        }
        ...
        console.log(
          `[OnboardingFlow] Resuming to step ${resumeStep} (saved currentStep: ${savedCurrentStep}, lastCompletedStep: ${saved.lastCompletedStep}, activeSteps: [${restoredActiveSteps.join(", ")}])`,
        );
        setStep(resumeStep);
        return resumeStep; // Return the step to resume from
      } else {
        console.log(
          "[OnboardingFlow] No saved progress found, starting from step 1",
        );
        return 1;
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    if (inviteToken && !isVerified && !checkingToken) {
      ...
      console.warn(
        "[OnboardingFlow] Invitation check failed but EmployeeRouteGuard allowed access. Proceeding with form.",
      );
      setIsVerified(true);
    }
  ...
  useEffect(() => {
    if (isVerified && !checkingToken) {
      ...
      const timer = setTimeout(() => {
        ...
        console.log(
          "[OnboardingFlow] Initial load complete. Current step:",
          step,
        );
      }, 1500);
      ...
      <div className="min-h-screen bg-[#f2f6f9] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 text-center max-w-md w-full shadow-sm">
          <div className="size-16 bg-gray-50 rounded-full flex items-center justify-center mb-4 mx-auto">
            <span className="material-symbols-outlined text-gray-400 text-4xl animate-spin">
              hourglass_empty
            </span>
          </div>
    ...
    if (file) {
      ...
      if (!file.type.startsWith("image/")) {
        setValidationErrors((prev) => ({
          ...prev,
          passportPhoto: "Please upload an image file (JPG, PNG)",
        }));
        return;
      }
      ...
      if (file.size > 5 * 1024 * 1024) {
        setValidationErrors((prev) => ({
          ...prev,
          passportPhoto: "Image size should be less than 5MB",
        }));
        return;
      }
      ...
      reader.onerror = () => {
        setValidationErrors((prev) => ({
          ...prev,
          passportPhoto: "Error reading file. Please try again.",
        }));
      };
      reader.readAsDataURL(file);
    } else {
  ...
  const submitApplication = async () => {
    ...
    const newCleaner: Cleaner = {
      ...
      shiftType:
        preferredShiftPattern === "Mornings Only"
          ? "Morning"
          : preferredShiftPattern === "Afternoons Only"
            ? "Evening"
            : preferredShiftPattern === "Evenings / Weekends"
              ? "Night"
        ...
        try {
          ...
          if (inviteToken) {
            try {
              ...
              console.log(
                "[OnboardingFlow] Progress cleared after successful submission",
              );
            } catch (err) {
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <div className="space-y-4 sm:space-y-6">
            <h2 className="text-l font-semibold border-b pb-2 sm:pb-4 capitalize tracking-wider text-gray-500 text-base">
              Step 1: Citizenship / Immigration Status
            </h2>
              ...
              <div className="flex flex-col gap-2 col-span-1 md:col-span-2">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start">
                  <div className="flex-shrink-0">
                    <label
                      className={`flex flex-col items-center justify-center w-32 h-40 border-2 border-dashed rounded-xl cursor-pointer transition-all overflow-hidden ${
                    ...
                    {passportPhotoFile && (
                      <div className="flex items-center gap-2 text-sm text-gray-500">
                        ...
                        <span className="text-gray-700 font-medium">
                          ...
                          {(passportPhotoFile.size / 1024).toFixed(2)} KB)
                        </span>
                        <button
                          type="button"
                          onClick={() => {
      ...
      case 3:
        return (
          <div className="space-y-6">
            <h2 className="text-l font-semibold border-b pb-2 sm:pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Right to Work Verification
            </h2>
            <div className="border-2 border-[#425361]/20 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="p-4 sm:p-8 space-y-6 bg-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8  ">
                  <div className="space-y-6">
                    <h4 className="text-sm font-semibold font-black text-[#000] uppercase tracking-widest mb-4 flex items-center gap-2">
                      Required Steps
                    </h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <label
                        className={`flex items-center justify-center gap-2 h-24 rounded-lg border-2 border-dashed transition-all cursor-pointer ${
      ...
      case 4:
        return (
          <div className="space-y-6">
            <h2 className="text-l font-semibold border-b pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Visa Type
            </h2>
            <div className="space-y-4">
              <p className="text-sm font-bold text-gray-700">
                What is your visa type? <span className="text-red-500">*</span>
              </p>
      ...
      case 5:
        return (
          <div className="space-y-8">
            <h2 className="text-l font-semibold border-b pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Student Visa Section
            </h2>
            ...
            <div className="space-y-6">
              <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 flex items-start gap-3">
                ...
                <div className="text-xs text-blue-900 font-medium leading-relaxed">
                  ...
                  <p className="opacity-80">
                    As a Student Visa holder, we must verify your term dates to
                    ensure compliance with UKVI working hour restrictions.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-black uppercase tracking-widest text-gray-700 text-[#135bec]">
                    Term Start Date *
                  </label>
                  <input
                    type="date"
              ...
              <div className="space-y-4">
                <label className="text-xs font-black uppercase tracking-widest text-gray-700">
                  Upload Official Term Dates Document *
                </label>
                <label
                  className={`p-8 border-2 border-dashed rounded-2xl transition-all cursor-pointer group flex flex-col items-center justify-center text-center ${
                ...
                >
                  ...
                  />
                  <span className="material-symbols-outlined text-4xl text-gray-300 group-hover:text-[#135bec] transition-colors mb-2">
                    upload_file
                  </span>
              ...
              <label
                className={`flex items-start gap-4 p-5 border-2 rounded-2xl cursor-pointer transition-all group ${
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[#135bec] font-bold mt-1">
                    I understand I can work a maximum of 20 hours per week
                    during term time.
                  </p>
                </div>
              </label>
      ...
      case 6:
        return (
          <div className="space-y-6">
            <h2 className="text-l font-semibold border-b pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Identity Proof
            </h2>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  {
                    title: "Passport",
                    sub: "Standard identity document",
                    icon: "menu_book",
                    key: "passport" as const,
                  },
                  {
                    title: "Biometric Residence Permit (BRP)",
                    sub: "Issued to non-UK nationals",
                    icon: "badge",
                    key: "brp" as const,
                  },
                  {
                    title: "UK Residence Card / Frontier Worker Permit",
                    sub: "For eligible non-UK residents",
                    icon: "perm_identity",
                    key: "residenceCard" as const,
                  },
                  {
                    title: "Driving Licence",
                    sub: "Full or Provisional",
                    icon: "directions_car",
                    key: "drivingLicence" as const,
                  },
                ].map((doc) => (
                  <div
                    key={doc.title}
                    className="p-4 sm:p-5 border-2 border-[#e7ebf3] rounded-2xl bg-white hover:border-[#000] transition-all cursor-pointer group shadow-sm flex items-start gap-4"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-base font-semibold font-black text-gray-900 leading-tight group-hover:text-[#000] transition-colors">
                        {doc.title}
                      </p>
              ...
              <div className="mt-8 pt-8 border-t border-gray-200">
                <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100 mb-4">
                  ...
                  <p className="text-base text-gray-500 mt-1">
                    Please upload your last 3 months of salary slips. This is a
                ...
                <label
                  className={`p-6 border-2 border-dashed rounded-2xl transition-all cursor-pointer group flex flex-col items-center justify-center text-center ${
                    validationErrors.salarySlip
                      ? "border-red-500 bg-red-50"
                ...
                >
                  ...
                  />
                  <div
                    className={`size-14 sm:size-16 ${uploadedFiles.salarySlip ? "bg-green-100 text-green-600" : "bg-[#425361] text-white"} rounded-full flex items-center justify-center mb-4 transition-colors`}
                  >
                    <span className="material-symbols-outlined text-3xl">
                      {uploadedFiles.salarySlip
                        ? "check_circle"
                  ...
                  <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-2">
                    {uploadedFiles.salarySlip
                      ...
                      : "Upload Last 3 Month Salary Slip"}
                  </h3>
      ...
      case 7:
        return (
          <div className="space-y-4 sm:space-y-8">
            <h2 className="text-l font-semibold border-b pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Employment Type
            </h2>
            <div className="space-y-6">
              <p className="text-base font-bold text-gray-800">
                ...
                {[
                  {
                    id: "Full-Time",
                    icon: "schedule",
                    desc: "Typical 35-40 hours per week",
                  },
                  {
                    id: "Part-Time",
                    icon: "calendar_today",
                  ...
                  >
                    ...
                    />
                    <div
                      className={`size-12 sm:size-14 rounded-full flex items-center justify-center transition-colors ${
                        workPreference === pref.id
                          ? "bg-[#425361] text-white"
                    ...
                    <div>
                      <p
                        className={`text-base font-semibold font-black ${workPreference === pref.id ? "text-[#000]" : "text-gray-900"}`}
                      >
                        {pref.id}
                      </p>
            ...
            <div className="space-y-8">
              ...
              {hasDBS === true && (
                <div className="animate-in slide-in-from-top-4 duration-500 space-y-6">
                  <label
                    className={`p-4 sm:p-8 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center text-center group transition-all cursor-pointer ${
                      validationErrors.dbsCertificate
                        ? "border-red-500 bg-red-50"
                  ...
                  >
                    ...
                    <h3 className="text-sm font-black font-bold text-gray-900 uppercase tracking-widest">
              ...
              {hasDBS === false && (
                <div className="animate-in fade-in duration-500 p-6 bg-gray-50 border border-gray-100 rounded-2xl flex items-center gap-4">
                  ...
                  <p className="text-xs font-medium text-gray-600 leading-relaxed">
                    Xpect Group will initiate a{" "}
                    <span className="font-bold text-gray-900">
                      Enhanced DBS Check
                    </span>{" "}
                    for you upon successful completion of your profile.
                  </p>
                </div>
              )}
            </div>
      ...
      case 9:
        return (
          <div className="space-y-6">
            <h2 className="text-l font-semibold border-b pb-4 uppercase tracking-wider text-gray-500 text-base">
              Step {displayStep}: Employment Preferences
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-base font-semibold text-gray-700">
          ...
          <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className="bg-blue-50/30 p-4 sm:p-6 rounded-2xl border border-black-100 mb-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-sm font-black text-[#34d399] uppercase tracking-widest">
                  Confirmation Required
                </h3>
              </div>
              <p className="text-[14px] text-gray-600 leading-relaxed">
                By submitting this application, you agree to the following
                mandatory statements. This is the final step in your
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  {
                    id: "rtw",
                    text: "I consent to Right-to-Work verification",
                  },
                  {
                    id: "approval",
                    text: "I understand employment is subject to approval",
                  },
                  {
                    id: "gdpr",
                    text: "I consent to secure storage of my data under UK GDPR",
                  },
                ].map((item) => (
                  <label
                    key={item.id}
            ...
            <div className="p-4 bg-gray-50 border border-gray-100 rounded-xl mt-8 flex flex-col sm:flex-row gap-2 items-center justify-between">
              ...
              <span className="text-[12px] text-gray-900 font-black font-bold">
  ...
  const checkStepValidity = (): boolean => {
    if (step === 1) {
      ...
      return !!(
        availabilityToStart &&
        availabilityToStart !== "Select Availability" &&
        preferredShiftPattern &&
  ...
  const validateStep = (): boolean => {
    ...
    if (step === 1) {
      ...
      if (!hasAgreedToHours) {
        errors.hasAgreedToHours =
          "You must agree to the working hours declaration";
        isValid = false;
      }
    } else if (step === 6) {
      const hasIdentityDoc =
    ...
    <div className="flex-1 flex flex-col items-center py-4 sm:py-10 px-4 animate-in fade-in zoom-in-95 duration-500">
      <div className="w-full max-w-[800px] flex flex-col gap-4 sm:gap-8">
        <div className="flex flex-col gap-1 text-center">
          <h1 className="text-2xl sm:text-3xl font-black font-bold text-[#0d121b]">
            Join Xpect Group
          </h1>
          <p className="text-[#4c669a] font-base">
            Complete your profile to join our professional team.
          </p>
        </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-[#e7ebf3] shadow-sm flex flex-col gap-4">
          <div className="flex justify-between items-center text-sm">
            <span className="font-bold text-[#34d399]">
              Onboarding Step {displayStep} of {totalSteps}
            </span>
