...
interface PPERecordsContextType {
  ...
  addInventoryItem: (
    data: Omit<PPEInventoryRecord, "id">,
  ) => Promise<PPEInventoryRecord>;
  updateInventoryItem: (
    id: string,
    updates: Partial<PPEInventoryRecord>,
  ) => Promise<void>;
  deleteInventoryItem: (id: string) => Promise<void>;
}

const PPERecordsContext = createContext<PPERecordsContextType | undefined>(
  undefined,
);
...
export const usePPERecords = () => {
  ...
  if (!ctx)
    throw new Error("usePPERecords must be used within PPERecordsProvider");
  return ctx;
};
...
export const PPERecordsProvider: React.FC<PPERecordsProviderProps> = ({
  children,
}) => {
  ...
  const refreshRecords = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch PPE records",
      );
      setRecords([]);
    }
  }, []);
  ...
  const refreshInventory = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch PPE inventory",
      );
      setInventory([]);
    }
  }, []);
  ...
  const addRecord = useCallback(
    async (data: Omit<PPEIssueRecord, "id">): Promise<PPEIssueRecord> => {
  ...
  const updateRecord = useCallback(
    async (id: string, updates: Partial<PPEIssueRecord>) => {
  ...
  const updateInventoryItem = useCallback(
    async (id: string, updates: Partial<PPEInventoryRecord>) => {
