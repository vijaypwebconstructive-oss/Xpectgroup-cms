...
const CleanersContext = createContext<CleanersContextType | undefined>(
  undefined,
);
...
export const CleanersProvider: React.FC<CleanersProviderProps> = ({
  children,
}) => {
  ...
  const fetchCleaners = async () => {
    try {
      ...
      if (errorMessage.includes("Cannot connect to server")) {
        console.warn(
          "Backend server appears to be offline. Please start the backend server.",
        );
      }
  ...
  const updateCleaner = async (id: string, updates: Partial<Cleaner>) => {
    try {
      setError(null);
      const updatedCleaner = (await api.cleaners.update(
        id,
        updates,
      )) as Cleaner;
      setCleaners((prev) =>
        prev.map((c) => (c.id === id ? updatedCleaner : c)),
      );
    } catch (err: any) {
