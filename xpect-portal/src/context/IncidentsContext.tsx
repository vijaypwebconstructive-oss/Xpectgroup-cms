...
interface IncidentsContextType {
  ...
  addCorrectiveAction: (
    incidentId: string,
    data: Omit<CorrectiveAction, "id" | "incidentId">,
...
export const IncidentsProvider: React.FC<IncidentsProviderProps> = ({
  children,
}) => {
  const [incidents, setIncidents] = useState<Incident[]>([]);
  const [actionsByIncident, setActionsByIncident] = useState<
  ...
  const loadActions = useCallback(
    async (incidentId: string): Promise<CorrectiveAction[]> => {
      try {
  ...
  const getActionsByIncident = useCallback(
    (incidentId: string) => actionsByIncident[incidentId] ?? [],
    [actionsByIncident],
  );

  const addIncident = useCallback(
    async (data: Omit<Incident, "id">): Promise<Incident> => {
  ...
  const updateCorrectiveAction = useCallback(
    async (
      ...
      const updated = await api.incidents.updateAction(
        incidentId,
        actionId,
        updates,
      );
      setActionsByIncident((prev) => ({
        ...prev,
        [incidentId]: (prev[incidentId] ?? []).map((a) =>
