...
const RiskCoshhContext = createContext<RiskCoshhContextType | undefined>(
  undefined,
);
...
export const useRiskCoshh = () => {
  ...
  if (!ctx)
    throw new Error("useRiskCoshh must be used within RiskCoshhProvider");
  return ctx;
};
...
export const RiskCoshhProvider: React.FC<RiskCoshhProviderProps> = ({
  children,
}) => {
  ...
  const refreshChemicals = useCallback(async () => {
    ...
    try {
      ...
      setChemicalsError(
        err instanceof Error ? err.message : "Failed to fetch chemicals",
      );
      setChemicals([]);
    } finally {
  ...
  const addRiskAssessment = useCallback(
    async (ra: Omit<RiskAssessment, "id">): Promise<RiskAssessment> => {
  ...
  const addRAMS = useCallback(
    async (r: Omit<RAMS, "id"> & { documentData?: string }): Promise<RAMS> => {
  ...
  const updateRAMS = useCallback(
    async (
      ...
      setRAMSList((prev) =>
        prev.map((r) =>
          r.id === id ? { ...r, ...updated, documentData: undefined } : r,
        ),
      );
      return updated;
    },
  ...
  const addChemical = useCallback(
    async (c: Omit<Chemical, "id">): Promise<Chemical> => {
  ...
  const addSDS = useCallback(
    async (s: Omit<SDS, "id"> & { documentData: string }): Promise<SDS> => {
  ...
  const getRiskById = useCallback(
    (id: string) => riskAssessments.find((r) => r.id === id),
    [riskAssessments],
  );
  const getRAMSById = useCallback(
    (id: string) => ramsList.find((r) => r.id === id),
    [ramsList],
  );
  ...
  const getSDSById = useCallback(
    (id: string) => sdsList.find((s) => s.id === id),
    [sdsList],
  );
  const getSDSByChemicalId = useCallback(
    (chemicalId: string) => sdsList.find((s) => s.chemicalId === chemicalId),
    [sdsList],
  );
