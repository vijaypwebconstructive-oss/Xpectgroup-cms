...
import type {
  ...
  SalarySlip,
  Invoice,
} from "../modules/finance-payroll/types";
...
interface FinanceContextType {
  ...
  updateQuotation: (
    id: string,
    updates: Partial<Omit<Quotation, "id">>,
  ) => Promise<void>;
...
export const FinanceProvider: React.FC<FinanceProviderProps> = ({
  ...
  const refreshPayroll = useCallback(
    async (params?: { month?: number; year?: number }) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch payroll",
        );
      }
    },
    [],
  ...
  const refreshSiteContracts = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch site contracts",
      );
      setSiteContracts([]);
    }
  }, []);
  ...
  const refreshSalarySlips = useCallback(
    async (params?: { cleanerId?: string }) => {
      try {
        setError(null);
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch salary slips",
        );
        setSalarySlips([]);
      }
    },
  ...
  const generatePayroll = useCallback(
    async (params?: { month?: number; year?: number }) => {
      ...
      setPayrollRecords((prev) => [
        ...arr,
        ...prev.filter((p) => !arr.some((c) => c.id === p.id)),
      ]);
      return arr;
    },
    [],
  ...
  const addSiteContract = useCallback(
    async (data: Omit<SiteContract, "id">) => {
  ...
  const updateSiteContract = useCallback(
    async (
      id: string,
      updates: Parameters<typeof api.finance.siteContracts.update>[1],
    ) => {
  ...
  const updateQuotation = useCallback(
    async (id: string, updates: Partial<Omit<Quotation, "id">>) => {
