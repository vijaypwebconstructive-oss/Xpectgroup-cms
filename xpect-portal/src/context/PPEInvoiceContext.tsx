...
interface PPEInvoiceContextType {
  ...
  addPPEInvoiceRecord: (
    record: Omit<PPEInvoiceRecord, "id" | "createdAt">,
  ) => Promise<PPEInvoiceRecord>;
  updatePPEInvoiceRecord: (
    id: string,
    updates: Partial<PPEInvoiceRecord>,
  ) => Promise<void>;
}

const PPEInvoiceContext = createContext<PPEInvoiceContextType | undefined>(
  undefined,
);
...
export const PPEInvoiceProvider: React.FC<PPEInvoiceProviderProps> = ({
  children,
}) => {
  const [ppeInvoiceRecords, setPPEInvoiceRecords] = useState<
    PPEInvoiceRecord[]
  >([]);
  ...
  const refreshInvoices = useCallback(async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch PPE invoices",
      );
  ...
  const addPPEInvoiceRecord = useCallback(
    async (
      record: Omit<PPEInvoiceRecord, "id" | "createdAt">,
    ): Promise<PPEInvoiceRecord> => {
  ...
  const updatePPEInvoiceRecord = useCallback(
    async (id: string, updates: Partial<PPEInvoiceRecord>) => {
      const updated = await api.ppe.updateInvoice(id, {
        emailStatus: updates.emailStatus,
      });
      setPPEInvoiceRecords((prev) =>
