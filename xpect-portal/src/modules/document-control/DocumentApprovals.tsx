...
const DocumentApprovals: React.FC<Props> = ({ onSelectDoc, onBack }) => {
  ...
  return (
    <div className="min-h-full bg-[#f6f7fb]">
      {/* Header */}
      <div className="bg-[#F6F7FB] sm:px-8 sm:py-5 px-4 py-3">
        <button
          onClick={onBack}
      ...
      <div className="sm:px-8 px-4 sm:py-6 py-3">
        ...
        <div className="mb-5 flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl px-5 py-4">
          ...
          <p className="text-sm text-blue-700">
            Review each document carefully before approving. Approved documents
            become live policy and will be visible to all relevant staff. Click
            a document title to view its full details.
          </p>
        </div>

        {pending.length === 0 ? (
          <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-16 text-center">
            <span className="material-symbols-outlined text-[56px] text-green-300 block mb-4">
              task_alt
            </span>
                ...
                <thead>
                  <tr className="border-b border-[#e7ebf3] bg-[#f6f7fb]">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-5 py-3 text-left text-xs font-semibold text-[#6b7a99] uppercase tracking-wide whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[#e7ebf3]">
                  {pending.map((doc) => (
                    ...
                    >
                      <td className="px-5 py-4">
                        ...
                        >
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="font-semibold text-[#2e4150] group-hover:underline max-w-[220px] truncate">
                              {doc.title}
                            </span>
                          </div>
                          <p className="text-xs text-[#6b7a99] mt-0.5 ml-[26px]">
                            {doc.department}
                          </p>
                        </button>
                      </td>
                      ...
                      <td className="px-5 py-4 text-[#0d121b] font-mono font-medium">
                        v{doc.version}
                      </td>
                      <td className="px-5 py-4">
