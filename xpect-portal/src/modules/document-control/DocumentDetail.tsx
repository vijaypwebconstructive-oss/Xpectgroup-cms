...
const InfoRow: React.FC<{ label: string; value: string }> = ({
  ...
  <div className="flex flex-col gap-0.5">
    <span className="text-xs font-semibold text-[#6b7a99] uppercase tracking-wide">
      {label}
    </span>
...
const DocumentDetail: React.FC<Props> = ({ docId, onBack }) => {
  ...
  const daysToReview = doc.nextReviewDate
    ? daysUntilDate(doc.nextReviewDate)
    : null;
  const isOverdue =
    daysToReview !== null && daysToReview <= 0 && doc.status === "approved";
      ...
      <div className="bg-[#F6F7FB] sm:px-8 px-4 sm:py-5 py-3">
        ...
        <div className="flex items-start gap-4 flex-wrap flex-col sm:flex-row">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-[#0d121b] leading-tight">
              {freshDoc.title}
            </h1>
            <div className="flex items-center gap-3 mt-1.5 flex-wrap ">
              ...
              <span className="text-xs text-[#6b7a99] font-mono font-medium">
                Version {freshDoc.version}
              </span>
              <span
                className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${statusStyle(freshDoc.status)}`}
              >
      ...
      {isOverdue && (
        <div className="mx-8 mt-5 flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-5 py-3">
          ...
          <p className="text-sm font-semibold text-red-700">
            Review overdue — this document was due for review on{" "}
          ...
          <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm sm:p-6 p-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
              ...
              <InfoRow
                ...
                value={formatDate(freshDoc.effectiveDate)}
              />
              <InfoRow
                label="Last Review"
                value={formatDate(freshDoc.lastReviewDate)}
              />
              <InfoRow
                label="Next Review Due"
                value={formatDate(freshDoc.nextReviewDate)}
              />
              <InfoRow
                label="Review Cycle"
                value={`Every ${freshDoc.reviewFrequencyMonths} months`}
              />
            </div>
            {freshDoc.description && (
              <div className="mt-4 pt-4 border-t border-[#e7ebf3]">
                <p className="text-xs font-semibold text-[#6b7a99] uppercase tracking-wide mb-1.5">
                  Description
                </p>
          ...
          <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm sm:p-6 p-3">
            ...
            {freshDoc.fileName ? (
              <div className="flex items-center gap-4 p-4 bg-[#f6f7fb] rounded-xl border border-[#e7ebf3]">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#0d121b] truncate">
                    {freshDoc.fileName}
                  </p>
                  <p className="text-xs text-[#6b7a99] mt-0.5">
                    {freshDoc.fileSize} · PDF Document
                  </p>
                </div>
              ...
              <div className="p-8 border-2 border-dashed border-[#e7ebf3] rounded-xl text-center">
                ...
                <button className="mt-3 text-sm font-semibold text-[#2e4150] hover:underline">
                  Upload File
                </button>
              </div>
            )}
                ...
                {freshDoc.versionHistory.map((ver, i) => {
                  ...
                  return (
                    <div key={i} className="relative flex gap-4 pl-10">
                      <div
                        className={`absolute left-0 top-0.5 w-9 h-9 rounded-full border-2 border-white bg-white flex items-center justify-center shadow-sm ${
                          i === 0 ? "ring-2 ring-[#2e4150]/20" : ""
                        }`}
                      >
                        <span
                          className={`material-symbols-outlined text-[18px] ${icon.cls}`}
                        >
                          {icon.icon}
                        </span>
                      </div>
                      <div className="flex-1 bg-[#f6f7fb] rounded-xl p-4 border border-[#e7ebf3]">
                        <div className="flex items-center justify-between gap-2 mb-1 flex-wrap">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-bold text-[#0d121b] font-mono">
                              v{ver.version}
                            </span>
                            {i === 0 && (
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#2e4150] text-white">
                                Current
                              </span>
                            )}
                          </div>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm sm:p-6 p-3">
            ...
            <p className="text-xs text-[#6b7a99] mb-5">
              Current status:
              <span
                className={`ml-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${statusStyle(freshDoc.status)}`}
              >
                {statusLabel(freshDoc.status)}
              </span>
            </p>
            ...
            {freshDoc.submittedBy && (
              <div className="mb-5 p-3 bg-[#f6f7fb] rounded-xl border border-[#e7ebf3]">
                ...
                <p className="text-sm font-semibold text-[#0d121b] mt-0.5">
                  {freshDoc.submittedBy}
                </p>
                {freshDoc.submittedDate && (
                  <p className="text-xs text-[#6b7a99] mt-0.5">
                    {formatDate(freshDoc.submittedDate)}
                  </p>
                )}
              </div>
            )}
              ...
              <div className="space-y-3">
                <p className="text-sm font-semibold text-[#0d121b]">
                  ...
                  {confirmAction === "changes" &&
                    "Request changes for this document?"}
                </p>
                <textarea
                  value={approvalNote}
            ...
            <div
              className={`rounded-xl border shadow-sm p-5 ${
                isOverdue
                  ...
                  : daysToReview !== null && daysToReview <= 30
