...
const StatCard: React.FC<{
  ...
  trend: string;
  onClick?: () => void;
}> = ({ label, value, icon, iconBg, active, borderColor, trend, onClick }) => (
  <button
    onClick={onClick}
...
const DocumentsLibrary: React.FC<Props> = ({
  onSelectDoc,
  onCreateDoc: _onCreateDoc,
  onNavigateApprovals,
  onNavigateReviews,
}) => {
  void _onCreateDoc;
  ...
  const docs = useMemo(() => {
    ...
    return list;
  }, [search, category, statusFilter, statsFilter, docsList]);
  ...
  const stats = [
    ...
    {
      label: "Approved",
      value: docsList.filter((d) => d.status === "approved").length,
      icon: "verified",
  ...
  return (
    <div className="min-h-full bg-[#f6f7fb]">
      {/* Header */}
      <div className="bg-[#F6F7FB] sm:px-8 px-4 sm:py-6 py-3">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-2xl font-bold text-[#0d121b]">
              Document Control
            </h1>
            <p className="text-base text-[#4c669a] mt-1">
              ISO 9001 Policy & Document Management
            </p>
          </div>
      ...
      <div className="sm:px-8 px-4 sm:py-6 py-3 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat) => (
            <StatCard
              ...
              onClick={() => {
                setStatsFilter(
                  stat.filter as "ALL" | "APPROVED" | "PENDING" | "OVERDUE",
                );

                //
                // REDIRECT / FILTER
                //
                if (stat.filter === "APPROVED") {
                  setStatus("approved");
                }

                if (stat.filter === "PENDING") {
                  setStatus("pending");

                  //
                  // OPTIONAL PAGE REDIRECT
                  //
                  onNavigateApprovals();
                }

                if (stat.filter === "OVERDUE") {
                  onNavigateReviews();
                }
        ...
        <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-4 flex flex-wrap gap-3 items-center">
          ...
          {(search || category || statusFilter || statsFilter !== "ALL") && (
            <button
              onClick={() => {
                setSearch("");
                setCategory("");
                      ...
                      >
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="font-semibold text-[#0d121b] max-w-[220px] truncate">
                              {doc.title}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 text-[#0d121b] font-mono font-medium">
                          v{doc.version}
                        </td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${badge.cls}`}
                          >
                            {badge.label}
                          </span>
                        </td>
                        <td className="px-4 py-4 text-[#6b7a99] whitespace-nowrap">
                          ...
                          {doc.nextReviewDate ? (
                            <span
                              className={`text-sm font-medium ${
                                daysUntilDate(doc.nextReviewDate) <= 0
                                  ? "text-red-600"
                                  : daysUntilDate(doc.nextReviewDate) <= 30
                                    ? "text-amber-600"
                                    : "text-[#6b7a99]"
                              }`}
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setField("reviewFrequencyMonths", Number(e.target.value))
                    }
              ...
              {form.effectiveDate && (
                <div className="p-3 bg-[#f6f7fb] rounded-xl border border-[#e7ebf3] text-sm text-[#6b7a99]">
                  ...
                  <span className="font-semibold text-[#0d121b]">
                    {new Date(
                      addMonths(form.effectiveDate, form.reviewFrequencyMonths),
              ...
              {/* File Upload */}
              <div>
                <label className="block text-xs font-semibold text-[#6b7a99] uppercase tracking-wide mb-2">
                  Upload Document
                </label>
                {form.file ? (
                  ...
                  <label className="cursor-pointer block">
                    <div className="p-6 border-2 border-dashed border-[#e7ebf3] rounded-xl text-center hover:border-[#2e4150]/40 hover:bg-[#f6f7fb] transition-all">
                      <span className="material-symbols-outlined text-[32px] text-[#e7ebf3] block mb-1">
                        upload_file
                      </span>
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-700 flex items-start gap-2">
                ...
                <span>
                  New documents are submitted with{" "}
                  <strong>Pending Approval</strong> status and will appear in
                  the Approvals queue for review.
                </span>
              </div>
            </div>
