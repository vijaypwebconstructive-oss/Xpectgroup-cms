...
const StatCard: React.FC<{
  ...
  onClick?: () => void;
  trend: string;
}> = ({ label, value, icon, bg, border, active, onClick, trend }) => (
  <button
    onClick={onClick}
    className={`bg-white rounded-2xl shadow-sm hover:shadow-md transition-all sm:p-5 p-4 flex flex-col items-start gap-4 border-b-[5px] text-left cursor-pointer ${
      active ? "" : ""
    }`}
    style={{
      borderBottomColor: active ? border : "transparent",
    }}
  >
    ...
    <div>
      ...
      <p className="text-3xl font-black font-bold text-[#0d121b] mt-1">
        {value}
      </p>
    </div>
  </button>
...
const IncidentsList: React.FC<Props> = ({
  onSelectIncident,
  onCreateIncident,
}) => {
  ...
  const filtered = useMemo(() => {
    ...
    // STATS FILTER
    //
    if (statsFilter === "ACCIDENT") {
      list = list.filter((i) => i.type === "Accident");
    }

    if (statsFilter === "NEAR_MISS") {
      list = list.filter((i) => i.type === "Near Miss");
    }

    if (statsFilter === "INJURY") {
      list = list.filter((i) => i.injuryOccurred);
    }

    return list;
  }, [search, typeFilter, statsFilter, incidents]);
  ...
  const stats = [
    ...
    {
      label: "Accidents",
      value: incidents.filter((i) => i.type === "Accident").length,
    ...
    {
      label: "Near Misses",
      value: incidents.filter((i) => i.type === "Near Miss").length,
      icon: "warning",
    ...
    {
      label: "Injuries",
      value: incidents.filter((i) => i.injuryOccurred).length,
  ...
  const hasFilters = search || typeFilter || statsFilter !== "ALL";

  const handleCreateClick = () => onCreateIncident();

  return (
    <div className="min-h-full bg-[#f8fafc] w-screen sm:w-full">
      {/* Header */}
      <div className="sm:px-8 px-4 sm:py-6 py-3">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-lg sm:text-2xl font-bold text-[#0d121b]">
              Incident Register
            </h1>
            <p className="text-base text-[#6b7a99] mt-1">
              Record and review workplace incidents
            </p>
          </div>
          <button
      ...
      <div className="sm:px-8 px-4 sm:py-6 py-3 space-y-6 sm:max-w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat) => (
            <StatCard
              ...
              border={stat.border}
              trend={stat.trend}
              active={statsFilter === stat.filter}
              onClick={() => {
                setStatsFilter(
                  stat.filter as "ALL" | "ACCIDENT" | "NEAR_MISS" | "INJURY",
                );

                //
                // FILTER REDIRECT
                //
                if (stat.filter === "ACCIDENT") {
                  setType("Accident");
                }

                if (stat.filter === "NEAR_MISS") {
                  setType("Near Miss");
                }
        ...
        <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-4 flex flex-col sm:flex-row flex-wrap gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              placeholder="Search by ID, reporter or site…"
              value={search}
                  ...
                  filtered.map((inc) => (
                    ...
                    >
                      <td className="px-4 py-4 whitespace-nowrap">
                        <p className="text-[#0d121b] font-medium">
                          {fmtDate(inc.date)}
                        </p>
                      ...
                      <td className="px-4 py-4 text-[#0d121b] max-w-[200px] truncate">
                        {inc.site}
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[#0d121b] whitespace-nowrap">
                            {inc.worker}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${typeBadge(inc.type)}`}
                        >
                          {inc.type}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${inc.injuryOccurred ? "bg-red-500" : "bg-green-500"}`}
                          />
                          {inc.injuryOccurred ? "Yes" : "No"}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-right">
                        <span className="text-sm font-semibold text-[#2e4150] hover:underline whitespace-nowrap">
                          View Details
                        </span>
                      </td>
                    </tr>
