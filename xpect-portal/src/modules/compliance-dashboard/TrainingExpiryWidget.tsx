...
const TrainingExpiryWidget: React.FC<TrainingExpiryWidgetProps> = ({
  ...
  const urgentCount = records.filter((r) => r.daysRemaining <= 90).length;
    ...
    <div className="bg-white rounded-xl border border-[#e7ebf3] flex flex-col">
      ...
      {records.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-10 text-center">
          <span className="material-symbols-outlined text-[36px] text-green-400 mb-2">
            check_circle
          </span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#e7ebf3]">
                <th className="text-left p-3 text-xs font-semibold text-[#6b7a99] uppercase tracking-wider">
                  Employee
                </th>
                <th className="text-left p-3 text-xs font-semibold text-[#6b7a99] uppercase tracking-wider">
                  Training
            ...
            <tbody>
              {records.map((record) => {
                ...
                return (
                  <tr
                    key={record.id}
                    className={`border-b border-[#e7ebf3] last:border-0 ${style.row}`}
                  >
                    <td className="p-3">
                      <div className="flex items-center gap-2">
