...
const KPICard: React.FC<{
  ...
  >
    ...
    <div>
      ...
      <p
        className={`sm:text-3xl text-2xl font-bold mt-0.5 ${alert ? "text-red-600" : "text-[#0d121b]"}`}
      >
        {value}
      </p>
...
const staffStillExists = (
  ...
  cleanerIds: Set<string>,
  cleanerNames: Set<string>,
): boolean => {
...
const ComplianceDashboard: React.FC<ComplianceDashboardProps> = ({
  ...
  const riskCoshhData = useMemo(
    () => ({ riskAssessments, ramsList }),
    [riskAssessments, ramsList],
  );
  const clientsSitesData = useMemo(
    () => ({ clients, assignments }),
    [clients, assignments],
  );
  const policyDocsData = useMemo(() => ({ documents }), [documents]);
  const ppeData = useMemo(
    () => ({ records: ppeRecords, inventory: ppeInventory }),
    [ppeRecords, ppeInventory],
  );
  const trainingData = useMemo(
    () => ({ trainingRecords: recordsForExistingStaff }),
    [recordsForExistingStaff],
  );
  ...
  const siteIssues = useMemo(
    () => buildSiteIssues(riskCoshhData, clientsSitesData),
    [riskCoshhData, clientsSitesData],
  );
  ...
  const staffSummary = useMemo(
    () => buildStaffSummary(assignments),
    [assignments],
  );
  const trainingExpiry = useMemo(
    () => buildTrainingExpiry(recordsForExistingStaff),
    [recordsForExistingStaff],
  );
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <div className="flex items-center gap-3 flex-col sm:flex-row mb-1">
              ...
              <div>
                <h1 className="sm:text-2xl text-xl font-bold text-[#0d121b]">
                  Dashboard
                </h1>
            ...
            {criticalAlerts > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 border border-red-200">
                ...
                <span className="text-sm font-bold text-red-700">
                  {criticalAlerts} critical alerts require action
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
              ...
              <DocumentStatusWidget
                ...
                onNavigateToDocControl={() =>
                  onNavigate("DOCUMENT_CONTROL" as AppView)
                }
              />
            </div>
            ...
            <div className="bg-white rounded-xl border border-[#e7ebf3] p-5">
              <div className="flex items-start sm:items-center justify-between mb-4 flex-col sm:flex-row gap-2">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-[20px] text-[#2e4150]">
                    analytics
                  </span>
                ...
                <span className="text-xs text-[#6b7a99] bg-[#f5f7fa] px-3 py-1 rounded-full">
                  ISO 45001 / ISO 9001
                </span>
              </div>
                ...
                {[
                  {
                    ...
                    pct:
                      staffSummary.total > 0
                        ? Math.round(
                            (staffSummary.compliant / staffSummary.total) * 100,
                          )
                        : 0,
                    stroke: "#22c55e",
                  },
                  {
                    ...
                    pct:
                      documentSummary.total > 0
                        ? Math.round(
                            (documentSummary.approved / documentSummary.total) *
                              100,
                          )
                        : 0,
                    stroke: "#3b82f6",
