...
const RAMSDetail: React.FC<Props> = ({ ramsId, onBack }) => {
  ...
  return (
    <div className="min-h-full bg-[#f6f7fb]">
      {/* Header */}
      <div className="bg-[#F6F7FB] px-8 py-5">
        <button
          onClick={onBack}
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-[#6b7a99] mb-6">
              Are you sure you want to delete &quot;{rams.siteName}&quot;? This
              ...
              <div className="space-y-3">
                {rams.workMethod.map((step, i) => (
                  <div key={i} className="flex gap-4">
                    ...
                    <p className="text-sm text-[#0d121b] leading-relaxed pt-1">
                      {step}
                    </p>
                  </div>
                ))}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <span className="text-sm text-[#0d121b]">
                          {" "}
                          {rest.join(":")}
                        </span>
                      </div>
            ...
            {rams.signedCopyAvailable ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {rams.signedDocumentFileName && (
                      <p className="text-xs text-[#6b7a99] mt-0.5">
                        File: {rams.signedDocumentFileName}
                      </p>
                    )}
                    {rams.signedDocumentUploadedAt && (
                      <p className="text-xs text-[#6b7a99]">
                        Uploaded: {fmt(rams.signedDocumentUploadedAt)}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="space-y-3">
                <div className="p-6 border-2 border-dashed border-[#e7ebf3] rounded-xl text-center">
                  <span className="material-symbols-outlined text-[32px] text-[#e7ebf3] block mb-2">
                    upload_file
                  </span>
                  <p className="text-sm font-semibold text-[#0d121b]">
                    No signed copy
                  ...
                  <p className="text-xs text-[#6b7a99] mt-1">
                    Upload signed RAMS document (PDF, PNG, JPG, JPEG)
                  </p>
                </div>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e7ebf3] shrink-0">
              <h3 className="text-lg font-bold text-[#0d121b] truncate pr-4">
                {rams.signedDocumentFileName ||
                  `Signed RAMS — ${rams.siteName}`}
              </h3>
              <button
                onClick={() => setShowDocViewer(false)}
                className="p-1.5 rounded-lg hover:bg-[#f6f7fb] transition-colors shrink-0"
              >
            ...
            <div className="flex-1 overflow-y-auto p-6 flex items-center justify-center bg-[#f8fafc] min-h-[300px]">
              {docData ? (
                ...
                <div className="prose prose-sm max-w-none text-[#0d121b] space-y-6">
                  ...
                  {rams.workMethod.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-[#6b7a99] uppercase tracking-wide mb-2">
                        Step-by-Step Method
                      </p>
                  ...
                  <p className="text-xs text-[#6b7a99] pt-4">
                    Last updated: {fmt(rams.lastUpdated)}
                  </p>
                </div>
              )}
            </div>
