...
const RAMSList: React.FC<Props> = ({ onSelectRAMS, onBack }) => {
  ...
  const validate = (): Record<string, string> => {
    ...
    if (!form.description.trim())
      e.description = "Work description is required.";
    if (!form.workingHours.trim())
      e.workingHours = "Working hours are required.";
  ...
  const handleSubmit = async () => {
    ...
    const newRAMSData = {
      ...
      workMethod: form.workMethod.trim()
        ? form.workMethod
            .trim()
            .split("\n")
            .filter((l) => l.trim())
    ...
    <div className="min-h-full bg-[#F6F7FB] w-screen sm:w-full sm:max-w-full">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-base text-[#4c669a]">
                Risk Assessment & Method Statements linked to client sites
              </p>
            </div>
          </div>
          <button
      ...
      <div className="sm:px-8 px-4 sm:py-6 py-3 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-4 flex flex-wrap gap-3 items-center">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatus(e.target.value as RAMSStatus | "")}
                ...
                {ramsLoading ? (
                  <tr>
                    <td colSpan={6} className="px-4 py-16 text-center">
                      ...
                      <p className="text-[#4c669a] font-medium">
                        Loading RAMS…
                      </p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-16 text-center">
                      <span className="material-symbols-outlined text-red-500 text-[48px] block mb-3">
                        error
                      </span>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={fieldCls("siteName")}
                    disabled={!form.clientName}
                  >
                    <option value="">Select site…</option>
                    {(form.clientName
                      ? (sitesByClient[
                          clients.find((c) => c.name === form.clientName)?.id ??
                            ""
                        ] ?? allSiteOptions)
                      : []
                    ).map((s) => (
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. Monday–Friday, 06:00–08:00"
                  value={form.workingHours}
                  onChange={(e) => setField("workingHours", e.target.value)}
                  className={fieldCls("workingHours")}
                />
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Describe the work activity covered by this RAMS…"
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setField("workMethod", e.target.value)}
                  className={`${fieldCls("workMethod")} resize-none`}
                />
                <p className="text-[10px] text-[#6b7a99] mt-1">
                  One step per line. These will appear as numbered steps.
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setField("emergencyProcedures", e.target.value)
                  }
                  className={`${fieldCls("emergencyProcedures")} resize-none`}
                />
                <p className="text-[10px] text-[#6b7a99] mt-1">
                  One procedure per line. Use "Type: Description" format.
                </p>
              </div>
            </div>
