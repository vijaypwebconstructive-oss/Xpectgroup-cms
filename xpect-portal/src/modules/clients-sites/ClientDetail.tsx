...
import {
  ...
  contractHealth,
  daysUntil,
} from "../../context/ClientsSitesContext";
...
const DOC_DEFINITIONS: DocEntry[] = [
  {
    ...
    icon: "description",
    desc: "Upload the signed SLA document.",
  },
  {
    key: "insurance",
    label: "Public Liability Insurance",
    icon: "shield",
    desc: "Current insurance certificate.",
  },
  {
    key: "risk",
    label: "Risk Assessment",
    icon: "health_and_safety",
    desc: "Site risk assessment document.",
  },
  {
    key: "contract",
...
const ClientDetail: React.FC<ClientDetailProps> = ({
  ...
  useEffect(() => {
    const fromApi = (client?.documents || []).reduce<
    ...
    <div className="flex-1 flex flex-col bg-[#f6f7fb] min-h-screen">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-[#6b7a99] text-sm mt-0.5">
              {client.industry} · {client.contactPerson}
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <span
              className={`inline-flex items-center px-3 py-1.5 rounded text-sm font-bold uppercase tracking-wide ${ss.badge}`}
            >
              {ss.label}
            </span>
            <button
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-[#6b7a99] mb-6">
              ...
              will also delete all associated sites and worker assignments. This
      ...
      <div className="flex-1 px-0 py-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-4">
            {[
              ...
              },
              {
                ...
                warn: daysUntil(client.contractEnd) < 30,
              },
              {
                label: "Insurance Expiry",
                value: formatDate(client.insuranceExpiry),
                warn: daysUntil(client.insuranceExpiry) < 30,
              },
              // { label: "Client Relation", value: client.relation },
            ].map((item) => (
              <div key={item.label} className="min-w-0">
                <p className="text-xs font-semibold text-[#6b7a99] mb-1">
                  {item.label}
                </p>
                <p
                  className={`text-sm font-bold truncate ${item.warn ? "text-amber-600" : "text-[#0d121b]"}`}
                  title={item.value}
                >
                  {item.value}
                </p>
        ...
        {activeTab === "overview" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-5 space-y-3">
              <h3 className="text-[#0d121b] font-black text-sm uppercase tracking-wide">
                Contract Summary
              </h3>
              {[
                ...
                <div
                  key={item.label}
                  className="flex items-center justify-between py-2 border-b border-[#f0f2f7] last:border-0"
                >
                  <p className="text-sm text-[#6b7a99] font-medium">
                    {item.label}
                  </p>
          ...
          <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm overflow-hidden">
            {sites.length === 0 ? (
              <div className="py-14 text-center">
                <span className="material-symbols-outlined text-[#c7c7c7] text-4xl block mb-2">
                  location_off
                </span>
              ...
              <table className="w-full min-w-[550px]">
                <thead>
                  <tr className="border-b border-[#e7ebf3] bg-[#f8fafc]">
                    <th className="text-left px-6 py-3 text-xs font-bold text-[#6b7a99] uppercase tracking-wider">
                      Site Name
                    </th>
                    <th className="text-left px-4 py-3 text-xs font-bold text-[#6b7a99] uppercase tracking-wider">
                      Address
                    </th>
                    <th className="text-left px-4 py-3 text-xs font-bold text-[#6b7a99] uppercase tracking-wider">
                      Risk
                    ...
                    <th className="text-right px-6 py-3 text-xs font-bold text-[#6b7a99] uppercase tracking-wider">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#f0f2f7]">
                  {sites.map((site) => (
                    ...
                    >
                      <td className="px-6 py-4 text-sm font-bold text-[#0d121b]">
                        {site.name}
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded text-xs font-bold uppercase ${RISK_STYLES[site.riskLevel]}`}
                        >
                          {site.riskLevel}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-sm font-bold text-[#0d121b]">
                        {site.activeWorkers}
                      </td>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-8 h-8 rounded-full ${a.workerAvatarColor} flex items-center justify-center shrink-0`}
                            >
                        ...
                        <td className="px-4 py-4 text-sm text-[#6b7a99] font-medium">
                          {a.siteName}
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-bold ${cs.badge}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${cs.dot}`}
                            />
                            {a.complianceStatus}
                          </span>
                        </td>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {DOC_DEFINITIONS.map((doc) => {
                ...
                return (
                  <div
                    key={doc.key}
                    className={`bg-white rounded-xl border shadow-sm p-5 flex items-start gap-4 transition-all ${uploaded ? "border-green-200" : "border-[#e7ebf3]"}`}
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${uploaded ? "bg-green-50" : "bg-[#f0f2f7]"}`}
                    >
                      <span
                        className={`material-symbols-outlined text-[20px] ${uploaded ? "text-green-600" : "text-[#6b7a99]"}`}
                      >
                        {doc.icon}
                      </span>
                    </div>
                          ...
                          <div className="flex items-center gap-2">
                            <label className="flex items-center gap-1 text-xs font-bold text-amber-600 hover:text-amber-800 transition-colors cursor-pointer">
                              ...
                              <input
                                ...
                                ref={(el) => {
                                  fileInputRefs.current[doc.key + "-re"] = el;
                                }}
                                onChange={(e) => {
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <span
                        className={`text-xs px-2 py-1 rounded font-medium ${uploaded ? "text-green-700 bg-green-50 border border-green-200" : "text-[#9aa5be] bg-[#f8fafc] border border-[#e7ebf3]"}`}
                      >
                      ...
                      <button
                        onClick={() =>
                          uploaded &&
                          setPreviewDoc({ label: doc.label, file: uploaded })
                        }
                        disabled={!uploaded}
                ...
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300">
                  <div className="flex items-center justify-between px-6 py-4 border-b border-[#e7ebf3]">
                    <div className="flex items-center gap-3 min-w-0">
                      <span className="material-symbols-outlined text-[22px] text-[#2e4150]">
                        description
                      </span>
                      <div className="min-w-0">
                        <h2 className="text-base font-bold text-[#0d121b] truncate">
                          {previewDoc.label}
                        </h2>
                        <p className="text-xs text-[#6b7a99]">
                          {previewDoc.file.name} ·{" "}
                          {formatFileSize(previewDoc.file.size)}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 overflow-auto p-6">
                    {previewDoc.file.type.startsWith("image/") ? (
                      <img
                        ...
                        className="max-w-full max-h-[60vh] mx-auto rounded-xl shadow-sm"
                      />
                    ) : previewDoc.file.type === "application/pdf" ? (
                      <iframe
                        src={previewDoc.file.url}
                        title={previewDoc.file.name}
                        className="w-full h-[60vh] rounded-xl border border-[#e7ebf3]"
                      />
                    ) : (
                      <div className="flex flex-col items-center justify-center py-16 text-center">
                        <span className="material-symbols-outlined text-[60px] text-[#c7d2e0] mb-3">
                          article
                        </span>
                        <p className="text-sm font-bold text-[#0d121b]">
                          {previewDoc.file.name}
                        ...
                        <p className="text-xs text-[#6b7a99] mt-1">
                          {formatFileSize(previewDoc.file.size)} · Uploaded{" "}
                          {previewDoc.file.uploadedAt}
                        </p>
        ...
        <p className="text-center text-xs text-[#9aa5be] pb-4">
          © 2024 Xpect Group. All worker records are encrypted and stored in
