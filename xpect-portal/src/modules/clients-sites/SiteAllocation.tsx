...
const mapCourseToRequired = (course: string): string | null => {
  ...
  if (c.includes("child safeguarding") || c.includes("safeguarding"))
  ...
  if (c.includes("working at height") || c.includes("height"))
...
export type PersonnelItem = WorkerAssignment & {
  location: string;
  avatar: string;
};
...
const evaluateCompliance = (
  worker: PersonnelItem,
  site: Site,
): ComplianceLevel => {
  const missing = site.requiredTrainings.filter(
    (t) => !worker.completedTrainings.includes(t),
  );
  if (missing.length === 0) return "Fully Compliant";
  if (missing.length < site.requiredTrainings.length)
...
const SiteAllocation: React.FC<SiteAllocationProps> = ({ onBack }) => {
  ...
  const personnel: PersonnelItem[] = useMemo(() => {
    ...
    return cleaners.map((c, i) => {
      const trained = trainingRecords.filter(
        (tr) =>
          nameLower(tr.name) === nameLower(c.name) && tr.status === "Trained",
      );
      const completedTrainings: string[] = [];
      trained.forEach((tr) => {
        const mapped = mapCourseToRequired(tr.course);
        if (mapped && !completedTrainings.includes(mapped))
          completedTrainings.push(mapped);
      });
      let complianceStatus: "Compliant" | "Expiring" | "Non-Compliant" =
        "Non-Compliant";
      if (completedTrainings.length > 0) {
        complianceStatus =
          completedTrainings.length >= 3 ? "Compliant" : "Expiring";
      }
      return {
        id: `personnel-${c.id}`,
        workerId: c.id,
  ...
  const filteredPersonnel = personnel.filter(
    (p) =>
      !workerSearch ||
      p.workerName.toLowerCase().includes(workerSearch.toLowerCase()),
  );

  const isAssignedToSelectedSite = (workerId: string) =>
    selectedSiteId &&
    allAssignments.some(
      (a) => a.workerId === workerId && a.siteId === selectedSiteId,
    );
  ...
  const handleAssign = (person: PersonnelItem) => {
    ...
    const level = evaluateCompliance(person, selectedSite);
    if (level === "Non-Compliant") return;
    if (level === "Partial Compliance") {
      setConfirmWorker(person);
      return;
    }
    doAssign(person);
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f2f6f9] min-h-screen max-w-screen  ">
      {/* ── Header ── */}
      <div className=" sm:px-8 px-4 sm:pt-8 pt-3 pb-0">
        ...
        <h1 className="text-[#0d121b] text-2xl font-black font-semibold sm:text-[28px]">
          Site Allocation
        </h1>
        <p className="text-[#4c669a] text-base mt-1">
          Assign qualified personnel to work locations based on compliance
          matrix.
        </p>
        <p className="text-ms font-bold text-[#333] capitalize  tracking-wider px-1 pt-3">
          Select Work Site
        </p>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-5 sm:px-8 px-4 sm:mt-6 mt-3 mb-8">
        ...
        <div className="lg:w-[380px] shrink-0 space-y-3">
            ...
            {visibleSites.map((site) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-4  cursor-pointer ${isSelected ? "bg-[#2e4150]/5 border-l-2 border-[#2e4150]/50" : "hover:bg-[#fafbfc]"}`}
                >
                  <div className="flex items-start gap-2">
                    <span
                      className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${RISK_DOT[site.riskLevel]}`}
                    />
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-bold ${isSelected ? "text-[#2e4150]" : "text-[#0d121b]"}`}
                      >
                        {site.name}
                      </p>
                      <p className="text-xs text-[#6b7a99] mt-0.5">
                        {cl?.address ?? site.address}
                      </p>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {site.requiredTrainings.slice(0, 3).map((t) => (
                          <span
                            key={t}
                            className="text-[10px] font-bold uppercase tracking-wide bg-[#e7ebf3] text-[#4c669a] px-1.5 py-0.5 rounded"
                          >
          ...
          <div className="rounded-xl bg-[#2e4150] text-white p-4 flex items-start gap-3">
            <span className="material-symbols-outlined text-[22px] text-white/80 mt-0.5 shrink-0">
              verified_user
            </span>
            <div>
              <p className="text-sm font-black mb-1">Compliance Rule</p>
              <p className="text-xs text-white/70 leading-relaxed">
                Assignment is strictly restricted to workers who hold all
                mandatory certifications required for the selected site.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1  overflow-x-scroll sm:min-w-0  max-w-[350px] sm:max-w-full ">
          <div className="rounded-xl border border-[#e7ebf3] shadow-sm bg-white h-full min-w-[500px]">
            ...
            <div className="flex items-center justify-between gap-3 sm:px-5 px-3 sm:py-4 py-3 border-b border-[#e7ebf3]">
              <p className="text-base font-black font-semibold text-[#0d121b]">
                Available Personnel
              </p>
              <label className="flex items-center h-9 bg-[#f6f7fb] rounded-lg px-3 border border-[#e7ebf3] focus-within:border-[#2e4150]/40 transition-all w-52">
                <span className="material-symbols-outlined text-[#9aa5be] text-[16px] mr-1.5">
                  search
                </span>
                <input
            ...
            <div className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-2 bg-[#f8fafc] border-b border-[#e7ebf3] ">
              <p className="text-xs font-bold text-[#6b7a99] uppercase tracking-wider">
                Worker
              </p>
              <p className="text-xs font-bold text-[#6b7a99] uppercase tracking-wider w-24 text-center">
                Assigned
              </p>
              <p className="text-xs font-bold text-[#6b7a99] uppercase tracking-wider w-44 text-center">
                Compliance Indicator
              </p>
              <p className="text-xs font-bold text-[#6b7a99] uppercase tracking-wider w-32 text-right">
                Action
              </p>
            </div>
            ...
            <div className="divide-y divide-[#f0f2f7]">
              {cleanersLoading ? (
                ...
                <div className="py-14 text-center">
                  <span className="material-symbols-outlined text-[#d1d5db] text-4xl block mb-2">
                    search_off
                  </span>
                ...
                filteredPersonnel.map((person) => {
                  const compliance = selectedSite
                    ? evaluateCompliance(person, selectedSite)
                    : "Non-Compliant";
                  ...
                  const missingTrainings = selectedSite
                    ? selectedSite.requiredTrainings.filter(
                        (t) => !person.completedTrainings.includes(t),
                      )
                    : [];
                  ...
                  return (
                    <div
                      key={person.id}
                      className="grid grid-cols-[1fr_auto_auto_auto] min-w-full gap-4 items-center px-5 py-4"
                    >
                      {/* Worker info */}
                      <div className="flex items-center gap-3 min-w-0">
                        <div
                          className={`w-10 h-10 rounded-full ${person.workerAvatarColor} flex items-center justify-center shrink-0 text-white text-sm font-black`}
                        >
                        ...
                        <div className="min-w-0">
                          ...
                          {missingTrainings.length > 0 && (
                            <p className="text-xs text-red-500 font-medium mt-0.5 truncate">
                              Missing: {missingTrainings.slice(0, 2).join(", ")}
                              {missingTrainings.length > 2
                                ? ` +${missingTrainings.length - 2}`
                                : ""}
                            </p>
                          )}
                      ...
                      <div className="w-24 flex justify-center">
                        <span className="text-xs font-semibold text-[#4c669a]">
                          {sitesLabel}
                        </span>
                      </div>
                      ...
                      <div className="w-44 flex justify-center">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-bold ${COMPLIANCE_BADGE[compliance]}`}
                        >
                          <span
                            className={`material-symbols-outlined text-[14px] ${COMPLIANCE_ICON_COLOR[compliance]}`}
                          >
                      ...
                      <div className="w-32 flex justify-end">
                        {isAssigned ? (
                          ...
                          <span className="text-xs font-bold text-[#9aa5be] uppercase tracking-wide">
                            Ineligible
                          </span>
                        )}
                      </div>
              ...
              <div className="border-t border-[#e7ebf3] px-5 py-3 bg-[#f8fafc]">
                ...
                <div className="flex flex-wrap gap-2">
                  {existingWorkers.map((w) => (
                    <div
                      key={w.id}
                      className="flex items-center gap-1.5 bg-white border border-[#e7ebf3] rounded-full px-2.5 py-1"
                    >
                      <div
                        className={`w-5 h-5 rounded-full ${w.workerAvatarColor} flex items-center justify-center text-white text-[9px] font-black`}
                      >
                        {w.workerInitials}
                      </div>
                      <span className="text-xs font-medium text-[#0d121b]">
                        {w.workerName}
                      </span>
                    </div>
                  ))}
      ...
      {confirmWorker && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 ">
            ...
            <h3 className="text-[#0d121b] font-black text-lg text-center mb-2">
              Partial Compliance
            </h3>
            <p className="text-[#6b7a99] text-sm text-center mb-6">
              <strong className="text-[#0d121b]">
                {confirmWorker.workerName}
              </strong>{" "}
              does not hold all required certifications for this site. Assigning
              them may breach compliance rules. Do you wish to proceed?
            </p>
