...
const ClientSitesModule: React.FC = () => {
  ...
  const renderContent = () => {
    switch (csState.view) {
      ...
      case "site-detail":
        return <SiteDetail siteId={csState.id ?? ""} onBack={goToSites} />;
      case "allocation":
  ...
  return (
    <div
      className={`flex-1 flex flex-col w-full animate-in slide-in-from-bottom-4 duration-500 min-h-[calc(100vh-160px)] ${csState.view === "allocation" ? "" : "py-[15px] sm:py-8 px-4 sm:px-6 md:px-10"}`}
    >
