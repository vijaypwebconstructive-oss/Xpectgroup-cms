...
const InvoiceForm: React.FC<Props> = ({
  ...
  const [serviceItems, setServiceItems] = useState(
    defaults.serviceItems ?? DEFAULT_SERVICE_ITEMS,
  );
  ...
  const defaultServiceDetails = Array.isArray(defaults.serviceDetails)
    ...
    : defaults.serviceDetails &&
        typeof defaults.serviceDetails === "object" &&
        "siteLocation" in defaults.serviceDetails
      ? [defaults.serviceDetails]
      : [{ siteLocation: "", siteType: "", supervisorName: "" }];
  ...
  const [invPrefix, setInvPrefix] = useState(
    templateDefaults?.invoicePrefix ?? "INV-YYYY",
  );
  const [vatPercent, setVatPercent] = useState(
    templateDefaults?.defaultVatPercent ?? 20,
  );
  const [defServiceCharges, setDefServiceCharges] = useState(
    templateDefaults?.defaultServiceCharges ?? 0,
  );
  const [paymentTermsDays, setPaymentTermsDays] = useState(
    templateDefaults?.defaultPaymentTermsDays ?? 14,
  );
  const [defNotes, setDefNotes] = useState(
    templateDefaults?.defaultNotes ?? "",
  );
  const [defFooter, setDefFooter] = useState(
    templateDefaults?.defaultFooter ?? "",
  );
      ...
      <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-6 sm:p-8 space-y-8">
        ...
        <div className="bg-[#fafbfd] rounded-xl border border-[#e7ebf3] p-4 sm:p-6 space-y-6">
          <CompanyInformationForm
            value={billBy}
            onChange={(v) => setCompany((prev) => ({ ...prev, ...v }))}
          />
        </div>
        ...
        {!isTemplate && (
          <div className="bg-[#fafbfd] rounded-xl border border-[#e7ebf3] p-4 sm:p-6 space-y-6">
            <InvoiceInformationForm
              value={invoiceInfo}
              onChange={(v) => setInvoiceInfo((prev) => ({ ...prev, ...v }))}
            />
          </div>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xs text-[#6b7a99] mt-1">
                  Use YYYY for year (e.g. INV-2026-001)
                </p>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#6b7a99] uppercase tracking-wide mb-1.5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDefServiceCharges(parseFloat(e.target.value) || 0)
                  }
                  className="w-full px-3 py-2 bg-[#f6f7fb] border border-[#e7ebf3] rounded-lg text-sm"
                />
              </div>
