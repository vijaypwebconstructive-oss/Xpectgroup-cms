...
const QuotationForm: React.FC<Props> = ({ mode, quotationId, initialData }) => {
  ...
  const [serviceCharges, setServiceCharges] = useState(
    defaults.serviceCharges ?? "0.00",
  );
  const [totalAmount, setTotalAmount] = useState(
    defaults.totalAmount ?? "6480.00",
  );
  const [payableAmount, setPayableAmount] = useState(
    defaults.payableAmount ?? "5400.00",
  );
  const [notes, setNotes] = useState(defaults.notes ?? "");
  const [footer, setFooter] = useState(defaults.footer ?? "");
  const [status, setStatus] = useState<QuotationFormData["status"]>(
    ...
    try {
      if (mode === "edit" && quotationId) {
        ...
        const created = await addQuotation(
          payload as Parameters<typeof addQuotation>[0],
        );
        await refreshQuotations();
      ...
      <div className="bg-white rounded-xl border border-[#e7ebf3] shadow-sm p-6 sm:p-8 space-y-8">
        ...
        <div className="bg-[#fafbfd] rounded-xl border border-[#e7ebf3] p-4 sm:p-6 space-y-6">
          <QuotationInformationForm
            value={quotationInfo}
            onChange={(v) => {
              setQuotationInfo((prev) => ({ ...prev, ...v }));
            }}
          />
        </div>

        <div className="bg-[#fafbfd] rounded-xl border border-[#e7ebf3] p-4 sm:p-6 space-y-6">
          <QuotationBillingForm
            billBy={billBy}
            billTo={billTo}
        ...
        <div className="bg-[#fafbfd] rounded-xl border border-[#e7ebf3] p-4 sm:p-6 space-y-6">
          <h2 className="text-sm font-bold text-[#6b7a99] uppercase tracking-wide">
            Status
          </h2>
          <select
            value={status}
            onChange={(e) =>
              setStatus(e.target.value as QuotationFormData["status"])
            }
            className="w-full max-w-[200px] px-3 py-2 bg-[#f6f7fb] border border-[#e7ebf3] rounded-lg text-sm"
          >
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-[#2e4150] text-white rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-60"
          >
            {submitLoading
              ? "Saving…"
              : mode === "edit"
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-white border border-[#e7ebf3] text-[#4c669a] rounded-lg text-sm font-semibold hover:bg-[#f2f6f9]"
          >
            Preview
          </button>
          <button
